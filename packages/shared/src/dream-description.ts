...
export function getDreamDescriptionFacts(text: string) {
  ...
  return {
    countertopPoints,
    shower: hasPositiveDescriptionFact(
      text,
      /\b(?:prysznic\w*|natrysk\w*|kabina\s+(?:bezbrodzikow\w*|walk[ -]in))\b/,
    ),
    ...
    customCarpentry: hasPositiveDescriptionFact(
      text,
      /\b(?:(?:mebl\w*|zabudow\w*|szaf\w*|kuchni\w*)\s+(?:(?:stolarsk\w*|kuchenn\w*|meblow\w*|wykonan\w*|zrobion\w*|robion\w*|z)\s+){0,3}(?:pod|na)\s+(?:wymiar|zamowienie)|(?:robion\w*|zrobion\w*|wykonan\w*|mebl\w*|zabudow\w*|szaf\w*|kuchni\w*)\b[^.!?;\n]{0,65}?\b(?:pod|na)\s+wymiar\b[^.!?;\n]{0,50}?\bstolar\w*|stolar\w*\b[^.!?;\n]{0,70}?\b(?:pod|na)\s+wymiar|(?:pod|na)\s+wymiar\b[^.!?;\n]{0,50}?\bstolar\w*)\b/,
    ),
    multipleParking,
...
export function getCountertopPoints(text: string) {
  const materials: Array<[string, number]> = [
    ["granit\\w*", 8],
    ["(?:konglomerat\\w*|spiek\\w*)", 7],
    ["(?:drewnian\\w*|drewn(?:o|a|em)|debow\\w*|dab|debu|debem|jesion\\w*|bukow\\w*)", 5],
  ];
  for (const [material, points] of materials) {
    const pattern = new RegExp(
      `\\b(?:blat\\w*\\b[^.!?;\\n]{0,60}?\\b${material}|${material}\\b[^.!?;\\n]{0,40}?\\bblat\\w*)\\b`,
    );
    if (hasPositiveDescriptionFact(text, pattern)) return points;
  }
  return 0;
}
