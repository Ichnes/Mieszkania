...
function hasFact(text: string, pattern: RegExp) {
...
export function getDreamDescriptionFacts(text: string) {
  ...
  const multipleParking = hasFact(text, multipleParkingPattern);
  ...
  const rentedMultipleParking =
    ...
    text
      ...
      .some(
        (clause) =>
          hasFact(clause, multipleParkingPattern) &&
          hasFact(clause, rentalPattern) &&
          !/\b(?:nie\s+(?:sa\s+)?(?:wynajm\w*|dzierzaw\w*)|bez\s+(?:oplat\s+miesieczn\w*|czynszu\s+najmu))\b/.test(
            clause,
          ),
      );
  return {
    shower: hasFact(text, /\b(?:prysznic\w*|natrysk\w*)\b/),
    topFloor: hasFact(
      text,
    ...
    stoneCountertop: hasFact(
      text,
      /\b(?:blat\w*\b[^.!?;\n]{0,90}?\b(?:spiek\w*|granit\w*|konglomerat\w*)|(?:spiek\w*|granit\w*|konglomerat\w*)\b[^.!?;\n]{0,50}?\bblat\w*)\b/,
    ),
    woodenFloor: hasFact(
      text,
      /\b(?:drewnian\w*\s+(?:podlog\w*|parkiet\w*)|podlog\w*[^.!?;\n]{0,70}?(?:drewn\w*|dab\w*\s+wedzon\w*)|egzotyczn\w*\s+drewn\w*|debow\w*\s+desk\w*|desk\w*\s+(?:debow\w*|drewnian\w*|podlogow\w*)|merbau\w*|parkiet\w*)\b/,
    ),
    customCarpentry: hasFact(
      text,
    ...
    unfinished: hasFact(text, /\b(?:do\s+wykonczenia|stan\w*\s+dewelopersk\w*)\b/),
  };
}
