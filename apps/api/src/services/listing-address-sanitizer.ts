...
import { normalizePolish, normalizeWarsawListingCity, sanitizeWarsawAddressText } from "./address-normalization";
...
export async function sanitizeStoredListingAddresses(options?: { dryRun?: boolean }) {
  return withDb(async (db) => {
    ...
    const changes = result.rows.flatMap((row) => {
      ...
      const inferredNeighborhood = canonicalWarsawNeighborhood(row.neighborhood, row.district)
        ?? inferWarsawNeighborhood([row.title, row.description, row.address_text].filter(Boolean).join(" "), row.district);
      const neighborhood = inferredNeighborhood && normalizePolish(inferredNeighborhood) !== normalizePolish(row.district ?? "")
        ? inferredNeighborhood
        : undefined;
      const addressText = sanitizeWarsawAddressText(row.address_text, { ...row, neighborhood, city }) ?? row.address_text;
      return addressText !== row.address_text || city !== row.city || neighborhood !== (row.neighborhood ?? undefined)
    ...
    await db.query(
      `
        ...
        where l.id = item.id
      `,
      [JSON.stringify(changes.map((change) => ({ id: change.id, address_text: change.addressText, city: change.city, neighborhood: change.neighborhood ?? null })))]
    );
