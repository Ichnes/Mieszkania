...
import { getMediaResponsePath } from "./image-repository";
...
const sourcePriority = ["otodom", "gratka", "morizon", "adresowo", "domiporta", "nieruchomosci_online", "maxon", "olx"];

export async function getDuplicateCandidates(limit = 50, listingId?: string): Promise<DuplicateCandidatesResponse> {
  ...
  const descriptionOverlap = descriptionTokenOverlapSql("coalesce(l1.description, '')", "coalesce(l2.description, '')");
  ...
  return withDb(async (db) => {
    ...
    try {
      ...
      if (input.status === "same_listing") {
        const primaryListingId = await choosePreferredPrimaryListingId(db, input.leftId, input.rightId);
...
export async function autoMergeDuplicateByDescription(
  db: QueryableDb,
  listingId: string
) {
  ...
  const best = candidatesResult.rows
    .map((candidate) => {
      const similarity = computeDescriptionSimilarity(listingTokens, tokenizeDescription(candidate.description));
      const matchingPrefixWords = countMatchingDescriptionPrefixWords(listingPrefixTokens, tokenizeDescriptionPrefix(candidate.description));
      return { candidate, similarity, matchingPrefixWords };
    })
    .filter((match) => match.matchingPrefixWords >= exactDescriptionPrefixWordCount || match.similarity >= automaticDescriptionSimilarityThreshold)
    .sort((left, right) => right.matchingPrefixWords - left.matchingPrefixWords || right.similarity - left.similarity)[0];
...
export async function runAutomaticDuplicateMergeByDescription(limit = 10_000) {
  ...
  return withDb(async (db) => {
    ...
    for (const listing of result.rows) {
      const prefix = tokenizeDescriptionPrefix(listing.description).slice(0, exactDescriptionPrefixWordCount);
    ...
    const candidateGroups = [...signatureGroups.values()].filter((group) => new Set(group.map((listing) => listing.source_key)).size > 1);
    ...
    for (let offset = 0; offset < candidateGroups.length; offset += concurrency) {
      ...
      const outcomes = await Promise.all(batch.map(async (group) => {
        ...
        const primary = ordered.find((listing) => listing.existing_primary_id === listing.id) ?? ordered[0];
        ...
        for (const duplicate of ordered.slice(1)) {
          ...
          await db.query(
            ...
            [pairKey, leftId, rightId, `Automatycznie polaczona: identyczne pierwsze ${exactDescriptionPrefixWordCount} slow opisu.`]
          );
...
export type DuplicateGroupOverview = {
  ...
  primaryListingId: string;
  primaryTitle: string;
  members: Array<{ id: string; title: string; sourceLabel: string; canonicalUrl?: string; thumbnailUrl?: string; priceLabel: string; areaLabel: string; isPrimary: boolean }>;
};

export async function getDuplicateGroupOverviews(limit?: number, summary?: false): Promise<DuplicateGroupOverview[]>;
export async function getDuplicateGroupOverviews(limit: number, summary: true): Promise<{ items: DuplicateGroupOverview[]; total: number; totalMembers: number; totalCopies: number }>;
export async function getDuplicateGroupOverviews(limit = 200, summary = false) {
  return withDb(async (db) => {
    const result = await db.query<{
      group_id: string; listing_id: string; is_primary: boolean; title: string; source_label: string | null; canonical_url: string; price_amount: string | null; area_sqm: string | null; thumbnail_storage_key: string | null; thumbnail_source_url: string | null;
    }>(`
    ...
    for (const row of result.rows) {
      ...
      const member = { id: row.listing_id, title: row.title, sourceLabel: row.source_label ?? "Portal", canonicalUrl: row.canonical_url ?? undefined, thumbnailUrl: row.thumbnail_storage_key ? getMediaResponsePath(row.thumbnail_storage_key) : row.thumbnail_source_url ?? undefined, priceLabel: formatCurrencyLabel(row.price_amount), areaLabel: formatAreaLabel(row.area_sqm), isPrimary: row.is_primary };
      if (existing) { existing.members.push(member); if (row.is_primary) { existing.primaryListingId = row.listing_id; existing.primaryTitle = row.title; } continue; }
      grouped.set(row.group_id, { groupId: row.group_id, primaryListingId: row.is_primary ? row.listing_id : "", primaryTitle: row.is_primary ? row.title : "Oferta powiązana", members: [member] });
    }
    const groups = [...grouped.values()].filter((group) => group.primaryListingId && group.members.length > 1);
    ...
    const totalMembers = new Set(groups.flatMap((group) => group.members.map((member) => member.id))).size;
    const totalCopies = new Set(groups.flatMap((group) => group.members.filter((member) => !member.isPrimary).map((member) => member.id))).size;
    return summary ? { items, total: groups.length, totalMembers, totalCopies } : items;
  });
}

export async function unmergeDuplicateListing(primaryListingId: string, duplicateListingId: string) {
  return withDb(async (pool) => {
    ...
    try {
      ...
      if (!group || group.is_primary) throw new Error("Nie można rozłączyć oferty głównej tą akcją.");
      ...
      await db.query(`delete from listing_duplicate_group_members where listing_id = $1::uuid`, [duplicateListingId]);
      await db.query(`update listings set hidden_duplicate_of_id = null, hidden_at = null, updated_at = now() where id = $1::uuid`, [duplicateListingId]);
      await db.query(`delete from listing_duplicate_groups g where g.id = $1::uuid and (select count(*) from listing_duplicate_group_members m where m.group_id = g.id) < 2`, [group.group_id]);
      await db.query("commit");
...
export async function confirmDuplicateGroup(primaryListingId: string) {
  return withDb(async (db) => {
    ...
    await db.query(`update listing_duplicate_groups set manually_confirmed = true where id = $1::uuid`, [groupId]);
...
function mapDuplicateCandidateRow(row: DuplicateCandidateRow): DuplicateCandidate {
  return {
    ...
    reasons: row.reason_summary.split(",").map((item) => item.trim()).filter(Boolean),
...
function mapDuplicatePreview(side: "left" | "right", row: DuplicateCandidateRow): DuplicateListingPreview {
...
async function mergeDuplicateGroup(
  ...
  const primaryGroup = existingGroups.rows.find((row) => row.listing_id === primaryListingId)?.group_id;
  const duplicateGroup = existingGroups.rows.find((row) => row.listing_id === duplicateListingId)?.group_id;
...
async function choosePreferredPrimaryListingId(db: QueryableDb, leftId: string, rightId: string) {
  ...
  return choosePrimaryListingId(
    { ...left, city: "", description: null, rooms: null, area_sqm: null, price_amount: null, address_text: null, existing_primary_id: null },
    { ...right, city: "", description: null, rooms: null, area_sqm: null, price_amount: null, address_text: null, existing_primary_id: null }
  );
}
