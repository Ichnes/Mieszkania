...
type OverpassElement = { type: "way" | "relation"; id: number; tags?: { name?: string; highway?: string }; geometry?: Array<{ lat: number; lon: number }> };
...
export async function importWarsawStreets() {
  ...
  const streets = (payload.elements ?? []).filter((item) => item.tags?.name && item.geometry && item.geometry.length > 1);
  let imported = 0;
  for (const street of streets) {
    ...
    await pool.query(
      ...
      [street.type, street.id, street.tags!.name!, normalizeStreet(street.tags!.name!), centerLat, centerLng, district, JSON.stringify({ type: "LineString", coordinates: geometry.map((point) => [point.lon, point.lat]) })]
    );
    imported += 1;
  }
...
function normalizeStreet(value: string) {
  return value.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/ł/g, "l").toLowerCase().replace(/\b(?:ul|ulica|al|aleja|pl|plac)\.?\s*/g, "").replace(/[^a-z0-9]+/g, " ").trim();
}
