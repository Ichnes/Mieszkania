...
function row(input: Partial<RelistingMatchRow> & Pick<RelistingMatchRow, "id" | "status">): RelistingMatchRow {
  return {
    id: input.id,
    status: input.status,
...
test("matches a newly added active offer to an older archived version and calculates its price change", () => {
  const matches = matchRelistedListingRows([
    ...
    row({ id: "new", status: "active", firstSeenAt: "2026-03-01T10:00:00.000Z", lastSeenAt: "2026-03-01T10:00:00.000Z", priceAmount: 1_140_000 })
  ]);
...
test("does not match different descriptions based only on physical parameters", () => {
  const matches = matchRelistedListingRows([
    ...
    row({ id: "new", status: "active", firstSeenAt: "2026-03-01T10:00:00.000Z", description: Array.from({ length: 40 }, (_, index) => `inne${index}`).join(" ") })
  ]);
...
test("chooses the most recent archived version when confidence is equal", () => {
  const matches = matchRelistedListingRows([
    ...
    row({ id: "latest", status: "removed", firstSeenAt: "2026-01-20T10:00:00.000Z", removedAt: "2026-02-20T10:00:00.000Z" }),
    row({ id: "new", status: "active", firstSeenAt: "2026-03-01T10:00:00.000Z" })
  ]);
