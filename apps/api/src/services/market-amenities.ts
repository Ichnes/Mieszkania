...
import { extractFeatures, resolveListingAmenities } from "./listing-repository";

export async function getMarketAmenityFilter(db: Pool, baseFilter: string, options: { elevator?: string; garage?: string }) {
  if (options.elevator !== "true" && options.garage !== "true") return "";
  const result = await db.query<{
    id: string; description: string | null; payload_raw: Record<string, unknown> | null;
    has_lift_override: boolean | null; has_garage_override: boolean | null;
  }>(`
  ...
  `);
  const ids = result.rows.filter(row => {
    const description = row.description ?? "";
    const features = extractFeatures({ description, snapshotPayload: row.payload_raw ?? undefined });
    const amenities = resolveListingAmenities(features, { lift: row.has_lift_override, garage: row.has_garage_override }, description);
    return (options.elevator !== "true" || amenities.lift === true) && (options.garage !== "true" || amenities.garage === true);
  }).map(row => row.id);
  ...
  if (ids.some(id => !/^[0-9a-f-]{36}$/i.test(id))) throw new Error("Invalid listing UUID in amenity filter");
