...
type GeocodeResult = { latitude: number; longitude: number; approximate?: boolean; precision: LocationPrecision };
...
export async function geocodeListing(input: {
  ...
  const normalizedAddress = normalizeAddressForGeocoding(input.addressText ?? normalizedStreet ?? input.street);
  const parsedAddress = normalizedAddress ? parsePolishAddress(normalizedAddress) : null;
  const districtQuery = [input.neighborhood, input.district, input.city, "Polska"].filter(Boolean).join(", ");
  const cityQuery = [input.city, "Polska"].filter(Boolean).join(", ");
  const cacheKey = buildCacheKey({ city: input.city, street: normalizedStreet ?? undefined, address: normalizedAddress, district: input.district, neighborhood: input.neighborhood });
  ...
  const candidates: Array<{ url: URL | null; precision: LocationPrecision }> = [
    { url: buildStructuredSearchUrl(parsedAddress), precision: parsedAddress?.houseNumber ? "exact_address" : "street" },
  ...
  if (fallback) await writeGeocodeCache(cacheKey, cityQuery, { ...fallback, precision: "district" });
  return fallback;
}

export async function reverseGeocodeListing(input: {
  latitude: number;
  longitude: number;
}) {
  ...
  const district =
    ...
    undefined;
  const neighborhood = canonicalWarsawNeighborhood(address.neighbourhood, district)
...
async function fetchGeocodeResult(url: URL, precision: LocationPrecision): Promise<GeocodeResult | null> {
  await waitForNominatimSlot();
  const response = await fetch(url, {
    headers: {
      "user-agent": process.env.NOMINATIM_USER_AGENT ?? "mieszkania-local-app/0.1",
...
async function readGeocodeCache(cacheKey: string): Promise<GeocodeResult | null> {
  const result = await pool.query<{ latitude: string | null; longitude: string | null; location_precision: LocationPrecision }>(
    `select latitude::text, longitude::text, location_precision from geocode_cache where cache_key = $1`, [cacheKey]
  ).catch(() => null);
  const row = result?.rows[0];
  if (!row?.latitude || !row.longitude) return null;
  return { latitude: Number(row.latitude), longitude: Number(row.longitude), approximate: row.location_precision !== "exact_address", precision: row.location_precision };
}
...
async function findWarsawStreet(street: string): Promise<GeocodeResult | null> {
  const result = await pool.query<{ center_lat: string; center_lng: string }>(
  ...
  const row = result?.rows[0];
  return row ? { latitude: Number(row.center_lat), longitude: Number(row.center_lng), approximate: true, precision: "street" } : null;
}

function buildCacheKey(input: { city: string; street?: string; address?: string; district?: string; neighborhood?: string }) {
  return normalizeForLookup([input.address, input.street, input.neighborhood, input.district, input.city].filter(Boolean).join(" | "));
}

function normalizeStreetName(value: string) {
  return normalizeForLookup(value).replace(/\b(?:ul|ulica|al|aleja|pl|plac)\.?\s*/g, "").replace(/[^a-z0-9]+/g, " ").trim();
}
...
function buildStructuredSearchUrl(
  ...
  const url = new URL("https://nominatim.openstreetmap.org/search");
  url.searchParams.set("street", parsed.houseNumber ? `${parsed.street} ${parsed.houseNumber}` : parsed.street);
...
function resolveApproximateWarsawPoint(input: {
  ...
  const points: Array<[string, { latitude: number; longitude: number; approximate: true }]> = [
    ...
    ["mokotow", { latitude: 52.1937, longitude: 21.0340, approximate: true }],
    ...
    ["praga poludnie", { latitude: 52.2380, longitude: 21.0838, approximate: true }],
    ...
    ["ursynow", { latitude: 52.1410, longitude: 21.0323, approximate: true }],
    ["wawer", { latitude: 52.2084, longitude: 21.1604, approximate: true }],
    ["wesola", { latitude: 52.2540, longitude: 21.2241, approximate: true }],
    ...
    ["zoliborz", { latitude: 52.2680, longitude: 20.9864, approximate: true }]
  ];
...
function parsePolishAddress(value: string) {
  const match = value.match(/^(?<street>.+?)\s+(?<houseNumber>\d+[A-Za-z0-9\/-]*)[, ]+\s*(?<postalCode>\d{2}-\d{3})\s+(?<city>.+)$/);
