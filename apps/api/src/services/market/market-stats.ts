  ...
  return withDb(async (db) => {
    ...
    const [
      ...
      db.query<{
        ...
        `select coalesce(nullif(trim(l.district),''),'Bez dzielnicy') district, coalesce(nullif(trim(l.neighborhood),''),'Nieustalona') neighborhood, count(*) filter (where l.status='active')::text active, round(avg(l.price_amount/nullif(l.area_sqm,0)) filter (where l.status='active' and l.price_amount>0 and l.area_sqm > 0))::text avg_price from listings l where lower(l.city)='warszawa' and ${aliasedStatsFilters} group by 1,2 having count(*) filter (where l.status='active') > 0 order by 1, count(*) filter (where l.status='active') desc`,
      ),
      db.query<{
        week: string;
        new_listings: string;
      ...
      order by w
    `),
      db.query<{ label: string; count: string }>(
      ...
      db.query<{ label: string; count: string }>(
        `select s.name label, count(*)::text count from listings l join sources s on s.id=l.source_id where lower(l.city)='warszawa' and l.first_seen_at >= now()-make_interval(days => ${periodDays}) and ${aliasedStatsFilters} group by s.name order by count(*) desc`,
      ),
      db.query<{ label: string; count: string }>(
        `select case l.market_type when 'primary' then 'Rynek pierwotny' else 'Rynek wtórny' end label, count(*)::text count from listings l where lower(l.city)='warszawa' and l.first_seen_at >= now()-make_interval(days => ${periodDays}) and ${aliasedStatsFilters} group by l.market_type order by count(*) desc`,
      ),
