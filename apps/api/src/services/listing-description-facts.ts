...
export function enrichListingFromDescription(listing: ParsedListing): ParsedListing {
  ...
  return {
    ...
    addressText: shouldNormalizeAddress ? [street, listing.district, listing.city].filter(Boolean).join(", ") : listing.addressText,
...
export function inferConstructionYear(value?: string, currentYear = new Date().getFullYear()) {
  ...
  const patterns = [
    ...
    new RegExp(`\\b(?:budyn|blok|kamienic|apartamentow|dom|osiedl|will|inwestyc)\\w*(?:\\s+\\w+){0,4}\\s+z\\s+${year}\\s*(?:rok\\w*|r\\.)`),
    new RegExp(`\\b(?:budyn|blok|kamienic|apartamentow|dom|osiedl|will)\\w*(?:\\s+\\w+){0,4}\\s+z\\s+\\w+\\s+z\\s+${year}\\s*(?:rok\\w*|r\\.)`),
    new RegExp(`\\b(?:budyn|blok|kamienic|apartamentow|dom|osiedl|will)\\w*\\s+${year}\\s+rok\\w*\\s+budow\\w*`),
    new RegExp(`\\b(?:wybudowan|zbudowan|wzniesion|powstal)\\w*(?:\\s+\\w+){0,5}\\s+(?:w\\s+)?${year}\\s*(?:r\\.?|rok\\w*)?\\b`),
    new RegExp(`\\b(?:budyn|blok|kamienic|apartamentow|dom|osiedl|will|inwestyc)\\w*(?:\\s+\\w+){0,6}\\s+odd(?:an|ano|any|ana|ane)\\w*\\s+do\\s+(?:uzytku|uzytkowania)(?:\\s+\\w+){0,5}\\s+(?:w\\s+)?${year}\\b`),
    new RegExp(`\\b(?:termin\\s+)?zakonczeni\\w*\\s+(?:budowy|inwestycji)[^\\d]{0,50}(?:\\d{1,2}[.\\s/-]+){0,2}${year}\\s*(?:r\\.?|rok\\w*)?\\b`),
    new RegExp(`\\b(?:yearbuilt|buildingyear|build_year|construction_year|constructionyear)\\b\\D{0,30}${year}\\b`)
  ];
...
export function inferStructuredConstructionYear(value: unknown, currentYear = new Date().getFullYear()): number | undefined {
  ...
  for (const [key, child] of Object.entries(value)) {
    if (/^(?:yearBuilt|buildingYear|build_year|construction_year|constructionYear)$/i.test(key)) {
      ...
      if (Number.isInteger(candidate) && candidate >= 1800 && candidate <= currentYear + 1) return candidate;
    }
  }
...
function extractStreetFromTitle(title: string) {
  const match = title.match(/(?:^|,)\s*((?:ul\.?|ulica|al\.?|aleja|pl\.?|plac)\s+[\p{L}0-9.' -]{2,80})\s*$/iu);
  return match?.[1]?.trim();
}
...
function isPlausibleStreet(value?: string) {
  ...
  return words.length > 0 && words.length <= 7 && value.length <= 70
    && !/\b(?:tramwaj|komunikacja|centrum|pozwala|dojazd|minut)\b/i.test(value);
}

function extractStreet(description?: string) {
  if (!description) return undefined;
  const match = description.match(/\b(?:ul\.?|ulica|al\.?|aleja|pl\.?|plac)\s+([A-ZĄĆĘŁŃÓŚŹŻ][\p{L}0-9.' -]{1,70}?)(?=,|\.|;|\n|\s{2,}|$)/u);
  return match?.[1]?.trim();
}
...
function inferFloorFacts(description?: string): { floor?: number; totalFloors?: number } {
  ...
  const single = text.match(/\b(?:na|polozon\w*\s+na|usytuowan\w*\s+na)\s+(parter|\d+)\.?\s*pietr\w*/);
