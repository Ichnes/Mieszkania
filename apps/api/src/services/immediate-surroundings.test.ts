...
test("retains waste and civic features beyond twelve nearby rail segments", () => {
  const elements = Array.from({ length: 20 }, (_, id) => ({ type: "node" as const, id, lat: 52, lon: 21, tags: { railway: "rail" } }));
  ...
  assert.ok(result.findings.some((finding) => finding.category === "civic" && finding.severity === "information"));
});
...
test("detects an industrial building crossing the 50 metre parcel surroundings", () => {
  const result = analyzeImmediateSurroundings([
    ...
    { type: "way", id: 20, nodes: [1, 2, 3, 4, 1], tags: { building: "industrial", name: "Hala produkcyjna" } }
  ], 52, 21);
