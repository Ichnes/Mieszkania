...
export async function importWarsawDistrictBoundaries() {
  ...
  if (!response.ok)
    throw new Error(`Overpass district import failed with status ${response.status}`);

  const payload = (await response.json()) as { elements?: Array<OverpassWay | OverpassRelation> };
  const elements = payload.elements ?? [];
  const ways = new Map(
    elements.filter((item): item is OverpassWay => item.type === "way").map((way) => [way.id, way]),
  );
...
export async function findWarsawDistrictAtPoint(
  latitude: number,
  longitude: number,
  db: Queryable = pool,
) {
  ...
  return (
    result.rows.find((district) =>
      pointInGeometry([longitude, latitude], district.geometry_geojson),
    )?.name ?? null
  );
}
...
export async function backfillWarsawListingDistricts() {
  ...
  try {
    ...
    let assigned = 0;
    const requiringRepair = listings.rows.filter(
      (listing) => !isCanonicalWarsawDistrict(listing.district),
    );
    for (const listing of requiringRepair) {
      const district = await findWarsawDistrictAtPoint(
        Number(listing.latitude),
        Number(listing.longitude),
        db,
      );
      if (!district) continue;
      await db.query(`update listings set district = $2, updated_at = now() where id = $1`, [
        listing.id,
        district,
      ]);
      assigned += 1;
    ...
    return {
      scanned: requiringRepair.length,
      assigned,
      unmatched: requiringRepair.length - assigned,
    };
  } catch (error) {
...
export function buildDistrictGeometry(
  ...
  const outer = stitchRings(
    relation.members
      ...
      .map((member) => ways.get(member.ref)?.geometry ?? [])
...
function toPositions(points: Array<{ lat: number; lon: number }>): Ring {
  ...
  while (pending.length) {
    ...
    while (joined && !samePoint(ring[0], ring.at(-1)!)) {
      ...
      const index = pending.findIndex(
        (segment) => samePoint(segment[0], end) || samePoint(segment.at(-1)!, end),
      );
      if (index >= 0) {
