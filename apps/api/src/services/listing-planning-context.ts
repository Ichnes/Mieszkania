...
export async function getListingPlanningContext(listingId: string, force = false): Promise<PlanningContextResponse | null> {
  ...
  if (parcelContext.status !== "available" || !parcelContext.parcel) {
    return {
      ...
      acts: [],
      message: parcelContext.status === "missing_location"
  ...
  if (!force && freshCache?.planning_json.cacheVersion === CACHE_VERSION) return mapCached(freshCache, false);

  const immediateSurroundingsPromise = getImmediateSurroundings(parcelContext.latitude!, parcelContext.longitude!);
  try {
    const [acts, immediateSurroundings] = await Promise.all([fetchUrbanRegistryActs(parcelId), immediateSurroundingsPromise]);
    ...
    if (immediateSurroundings.status === "available") await saveCache(listingId, parcelId, response);
    return response;
  } catch {
    const immediateSurroundings = await immediateSurroundingsPromise;
    const staleCache = freshCache?.planning_json.cacheVersion === CACHE_VERSION ? freshCache : await readCache(listingId, parcelId, true);
    if (staleCache?.planning_json.cacheVersion === CACHE_VERSION) return { ...mapCached(staleCache, true), immediateSurroundings: immediateSurroundings.status === "available" ? immediateSurroundings : staleCache.planning_json.immediateSurroundings };
    return {
      status: "unavailable",
      provider: "Rejestr Urbanistyczny",
      parcelId,
      acts: [],
      message: "Rejestr Urbanistyczny lub jego usługa identyfikacji działek chwilowo nie odpowiada.",
      registryUrl: REGISTRY_URL,
      immediateSurroundings
    };
  }
