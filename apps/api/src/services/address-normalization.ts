...
export function normalizeWarsawStreetCandidate(value?: string | null) {
  ...
  const declined = [...knownWarsawStreetNames.entries()].find(([key]) => canonicalStreetForm(key) === canonicalStreetForm(normalized!));
...
export function sanitizeWarsawAddressText(
  ...
  return Array.from(new Map(cleanedParts.map((part) => [normalizePolish(part), part])).values()).join(", ");
}
...
function isNarrativeAddressPart(value: string) {
  ...
  return (
    ...
    || /\b(?:mieszkanie|lokal|apartament|nieruchomosc|dom|budynek|oferta) (?:jest|znajduje|polozon|usytuowan|sklada|oferuje|stanowi|posiada)/.test(normalized)
