...
import { canonicalWarsawDistrict, canonicalWarsawNeighborhood, inferWarsawNeighborhood } from "./warsaw-neighborhoods";
import { getMarketSignals } from "./market-signals";
import { getMarketAmenityFilter } from "./market-amenities";

export type MarketStatsQuery = { minYear?: string; minArea?: string; maxArea?: string; elevator?: string; garage?: string; period?: string };
...
return withDb(async (db) => {
  ...
  const periodDays = ([30, 90, 180].includes(requestedPeriod) ? requestedPeriod : 90) as 30 | 90 | 180;
  const scopeFilters = [`price_amount >= ${searchContract.minPrice}`, `price_amount <= ${searchContract.maxPrice}`, `rooms >= ${searchContract.roomsMin}`];
  const statsFilters = [...scopeFilters, "hidden_duplicate_of_id is null", "coalesce(exclusion_reason, '') <> 'manual_rejected'", Number.isFinite(minYear) && minYear > 0 ? `year_built >= ${Math.floor(minYear)}` : "", Number.isFinite(minArea) && minArea > 0 ? `area_sqm >= ${minArea}` : "", Number.isFinite(maxArea) && maxArea > 0 ? `area_sqm <= ${maxArea}` : ""].filter(Boolean).join(" and ");
  let aliasedStatsFilters = statsFilters.replace(/\b(hidden_duplicate_of_id|year_built|area_sqm|description|price_amount|rooms|exclusion_reason)\b/g, "l.$1");
  aliasedStatsFilters += await getMarketAmenityFilter(db, aliasedStatsFilters, query);
  const [totals, districts, neighborhoods, activity, distribution, comparison, roomSegments, areaSegments, buildingAgeSegments, sourceSegments, marketTypeSegments, boundaries, signals] = await Promise.all([
    db.query<{ active: string; archived: string; avg_price: string | null; median_price: string | null; avg_area: string | null; new_7: string; archived_30: string }>(`select count(*) filter (where status='active')::text active, count(*) filter (where status='removed' and coalesce(exclusion_reason,'') <> 'manual_rejected')::text archived, round(avg(price_amount/nullif(area_sqm,0)) filter (where status='active' and price_amount>0 and area_sqm > 0))::text avg_price, percentile_cont(0.5) within group (order by price_amount/nullif(area_sqm,0)) filter (where status='active' and price_amount>0 and area_sqm > 0) median_price, round(avg(area_sqm) filter (where status='active' and area_sqm > 0),1)::text avg_area, count(*) filter (where status='active' and first_seen_at >= now()-interval '7 days')::text new_7, count(*) filter (where status='removed' and coalesce(exclusion_reason,'') <> 'manual_rejected' and removed_at >= now()-interval '30 days')::text archived_30 from listings l where lower(l.city)='warszawa' and ${aliasedStatsFilters}`),
    db.query<{ district: string | null; active: string; archived: string; avg_price: string | null; median_price: string | null; priced: string; avg_area: string | null; drops: string; increases: string; new_7: string; new_period: string; archived_period: string; median_days: string | null }>(`select coalesce(nullif(trim(l.district),''),'Bez dzielnicy') district, count(*) filter (where l.status='active')::text active, count(*) filter (where l.status='removed' and coalesce(l.exclusion_reason,'') <> 'manual_rejected')::text archived, round(avg(l.price_amount/nullif(l.area_sqm,0)) filter (where l.status='active' and l.price_amount>0 and l.area_sqm > 0))::text avg_price, percentile_cont(0.5) within group (order by l.price_amount/nullif(l.area_sqm,0)) filter (where l.first_seen_at >= now()-make_interval(days => ${periodDays}) and l.price_amount>0 and l.area_sqm > 0)::text median_price, count(*) filter (where l.first_seen_at >= now()-make_interval(days => ${periodDays}) and l.price_amount>0 and l.area_sqm > 0)::text priced, round(avg(l.area_sqm) filter (where l.status='active' and l.area_sqm > 0),1)::text avg_area, count(*) filter (where l.status='active' and exists (select 1 from price_events pe where pe.listing_id=l.id and pe.event_type='price_drop' and pe.changed_at >= now()-make_interval(days => ${periodDays})))::text drops, count(*) filter (where l.status='active' and exists (select 1 from price_events pe where pe.listing_id=l.id and pe.event_type='price_increase' and pe.changed_at >= now()-make_interval(days => ${periodDays})))::text increases, count(*) filter (where l.status='active' and l.first_seen_at >= now()-interval '7 days')::text new_7, count(*) filter (where l.first_seen_at >= now()-make_interval(days => ${periodDays}))::text new_period, count(*) filter (where l.status='removed' and coalesce(l.exclusion_reason,'') <> 'manual_rejected' and l.removed_at >= now()-make_interval(days => ${periodDays}))::text archived_period, percentile_cont(0.5) within group (order by extract(epoch from (l.removed_at-l.first_seen_at))/86400) filter (where l.status='removed' and l.removed_at >= now()-make_interval(days => ${periodDays}) and l.removed_at >= l.first_seen_at)::text median_days from listings l where lower(l.city)='warszawa' and ${aliasedStatsFilters} group by 1 order by count(*) filter (where l.status='active') desc`),
    db.query<{ district: string; neighborhood: string; active: string; avg_price: string | null }>(`select coalesce(nullif(trim(l.district),''),'Bez dzielnicy') district, coalesce(nullif(trim(l.neighborhood),''),'Nieustalona') neighborhood, count(*) filter (where l.status='active')::text active, round(avg(l.price_amount/nullif(l.area_sqm,0)) filter (where l.status='active' and l.price_amount>0 and l.area_sqm > 0))::text avg_price from listings l where lower(l.city)='warszawa' and ${aliasedStatsFilters} group by 1,2 having count(*) filter (where l.status='active') > 0 order by 1, count(*) filter (where l.status='active') desc`),
    db.query<{ week: string; new_listings: string; archived_listings: string; median_price: string | null }>(`
      select
  ...
  const mapSegments = (rows: Array<{ label: string; count: string }>) => {
    const total = rows.reduce((sum, row) => sum + Number(row.count), 0);
    return rows.map((row) => ({ label: row.label, count: Number(row.count), sharePercent: total ? Math.round(Number(row.count) / total * 1_000) / 10 : 0 }));
  };
  return {
    periodDays,
    ...
    comparison: {
      ...
      priceDrops,
      priceDropSharePercent: Number(t.active) > 0 ? Math.round(priceDrops / Number(t.active) * 1_000) / 10 : 0,
    ...
    districts: districts.rows.map((row) => {
      ...
      const neighborhoodFromDistrict = canonicalWarsawNeighborhood(storedDistrict, district)
      ...
      return {
        ...
        geometry: boundaries.rows.find((boundary) => boundary.name === district)?.geometry_geojson,
    ...
    activity: activity.rows.map((row) => ({ week: row.week, newListings: Number(row.new_listings ?? 0), archivedListings: Number(row.archived_listings ?? 0), medianPricePerSqm: Number(row.median_price ?? 0) })),
    priceDistribution: distribution.rows.map((row) => ({ label: row.label, count: Number(row.count), sharePercent: distributionTotal ? Math.round(Number(row.count) / distributionTotal * 1_000) / 10 : 0 })),
    segments: {
