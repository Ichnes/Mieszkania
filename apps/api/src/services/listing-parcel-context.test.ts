...
import { parcelGeometryContainsPoint, parseUldkParcelResponse, parseWktGeometry } from "./listing-parcel-context";
...
test("parses the official ULDK pipe-delimited parcel response", () => {
  ...
  assert.deepEqual(result?.geometry, {
    type: "Polygon",
    coordinates: [[[21.01, 52.22], [21.02, 52.22], [21.01, 52.22]]]
  });
});
...
test("reuses a cached parcel only when the listing point is inside its geometry", () => {
  const geometry = parseWktGeometry("POLYGON((21.00 52.20,21.02 52.20,21.02 52.22,21.00 52.22,21.00 52.20))");
