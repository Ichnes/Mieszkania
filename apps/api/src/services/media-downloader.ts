...
export async function downloadListingMedia(assets: PendingMediaAsset[]) {
  ...
  for (let index = 0; index < assets.length; index += concurrency) {
    const batch = await Promise.all(assets.slice(index, index + concurrency).map(downloadSingleMedia));
    results.push(...batch);
  }
...
async function downloadSingleMedia(asset: PendingMediaAsset) {
  try {
    ...
    if (!response.ok) {
      ...
      return { assetId: asset.assetId, status: "failed" as const, reason: `HTTP ${response.status}` };
    }

    const bytes = Buffer.from(await response.arrayBuffer());
    const mimeType = response.headers.get("content-type") ?? inferMimeType(asset.sourceUrl);
    const outputPath = resolve(mediaCacheRoot, `${asset.storageKey}${getExtension(asset.sourceUrl, mimeType)}`);
...
function fetchTrustedGratkaMedia(url: string, headers: Record<string, string>, redirectsLeft = 4): Promise<Response> {
  return new Promise((resolveResponse, rejectResponse) => {
    const request = httpsRequest(url, { method: "GET", headers, rejectUnauthorized: false, timeout: 20_000 }, (response) => {
      ...
      if (location && status >= 300 && status < 400 && redirectsLeft > 0) {
        ...
        if (!isGratkaMediaHost(redirectedUrl.hostname)) {
          rejectResponse(new Error(`Refusing untrusted Gratka media redirect to ${redirectedUrl.hostname}`));
          return;
        }
        void fetchTrustedGratkaMedia(redirectedUrl.toString(), headers, redirectsLeft - 1).then(resolveResponse, rejectResponse);
        return;
      }
...
function isGratkaMediaHost(hostname: string) {
  return hostname === "thumbs.cdngr.pl"
