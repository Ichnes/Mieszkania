...
test("maps a registry response to a direct plan details link", () => {
  ...
  assert.equal(result?.detailsUrl, "https://rejestr-urbanistyczny.gov.pl/published/details/mpzp/1248");
});

test("maps a municipal general plan to the POG details route", () => {
  const result = mapUrbanRegistryAct({ publishedActId: 99, planType: "MUNICIPAL_GENERAL_PLAN", title: "Plan ogólny" });
