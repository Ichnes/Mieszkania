import { normalizePolish } from "./address-normalization";
...
export function inferWarsawDistrictFromText(value?: string | null) {
  const normalized = normalizePolish(value ?? "").replace(/[^\p{L}\p{N}]+/gu, " ").trim();
  return warsawDistrictAliases.find(([alias]) => new RegExp(`(?:^|\\s)${alias.replace(/ /g, "\\s+")}(?=$|\\s)`, "i").test(normalized))?.[1];
}
...
export function extractStreetFromLocationTitle(title: string, district?: string, city?: string) {
  ...
  if (segments.length < 2) {
    const inline = title.match(/\b(?:przy|ul\.?|ulica|al\.?|aleja|pl\.?|placu|plac)\s+([A-ZŻŹĆĄŚĘŁÓŃ][\p{L}-]*(?:\s+[A-ZŻŹĆĄŚĘŁÓŃ][\p{L}-]*)?)/u);
  ...
  const candidate = segments.at(-1)
    ...
    .trim();
  if (!candidate || candidate.length < 2 || candidate.length > 70 || candidate.split(/\s+/).length > 7) return undefined;

  const normalizedCandidate = normalizePolish(candidate);
  const excludedLocations = [district, city, "Warszawa", "Polska"]
    .filter((value): value is string => Boolean(value))
    .map(normalizePolish);
  if (excludedLocations.includes(normalizedCandidate) || inferWarsawDistrictFromText(candidate)) return undefined;
  if (/\b(?:mieszkanie|apartament|dom|sprzedaż|wynajem|pokojowe|pokoje)\b/i.test(candidate)) return undefined;
