...
const warsawNeighborhoods: WarsawNeighborhood[] = [
  ...inDistrict("Bemowo", ["Lotnisko", "Bemowo-Lotnisko", "Boernerowo", "Chrzanów", "Fort Bema", "Fort Radiowo", "Górce", "Groty", "Jelonki", "Jelonki Północne", "Jelonki Południowe"]),
  ...inDistrict("Białołęka", ["Białołęka Dworska", "Brzeziny", "Choszczówka", "Dąbrówka Szlachecka", "Grodzisk", "Henryków", "Kobiałka", "Marcelin", "Nowodwory", "Płudy", "Szamocin", "Tarchomin", "Wiśniewo", "Żerań"]),
  ...inDistrict("Bielany", ["Chomiczówka", "Huta", "Las Bielański", "Marymont-Kaskada", "Marymont-Ruda", "Młociny", "Piaski", "Placówka", "Radiowo", "Stare Bielany", "Słodowiec", "Wawrzyszew", "Wólka Węglowa", "Wrzeciono"]),
  ...inDistrict("Mokotów", ["Augustówka", "Czerniaków", "Ksawerów", "Sadyba", "Siekierki", "Sielce", "Służew", "Służewiec", "Stary Mokotów", "Stegny", "Wierzbno", "Wyględów"]),
  ...inDistrict("Ochota", ["Filtry", "Rakowiec", "Stara Ochota", "Szczęśliwice"]),
  ...inDistrict("Praga-Południe", ["Gocław", "Gocławek", "Grochów", "Kamionek", "Olszynka Grochowska", "Saska Kępa"]),
  ...inDistrict("Praga-Północ", ["Nowa Praga", "Pelcowizna", "Stara Praga", "Szmulowizna"]),
  ...inDistrict("Rembertów", ["Kawęczyn-Wygoda", "Nowy Rembertów", "Pocisk", "Polanka", "Stary Rembertów"]),
  ...inDistrict("Śródmieście", ["Muranów", "Nowe Miasto", "Powiśle", "Solec", "Stare Miasto", "Śródmieście Południowe", "Śródmieście Północne", "Ujazdów"]),
  ...inDistrict("Targówek", ["Bródno", "Bródno-Podgrodzie", "Elsnerów", "Targówek Fabryczny", "Targówek Mieszkaniowy", "Utrata", "Zacisze"]),
  ...inDistrict("Ursus", ["Czechowice", "Gołąbki", "Niedźwiadek", "Skorosze", "Szamoty"]),
  ...inDistrict("Ursynów", ["Dąbrówka", "Grabów", "Imielin", "Jeziorki Południowe", "Jeziorki Północne", "Kabaty", "Natolin", "Pyry", "Skarpa Powsińska", "Stary Imielin", "Stary Służew", "Stokłosy", "Ursynów-Centrum", "Ursynów Północny", "Wyczółki"]),
  ...inDistrict("Wawer", ["Aleksandrów", "Anin", "Falenica", "Las", "Marysin Wawerski", "Miedzeszyn", "Międzylesie", "Nadwiśle", "Radość", "Sadul", "Wawer", "Zerzeń"]),
  ...inDistrict("Wesoła", ["Groszówka", "Grzybowa", "Plac Wojska Polskiego", "Stara Miłosna", "Wesoła-Centrum", "Wola Grzybowska", "Zielona", "Zielona-Grzybowa"]),
  ...inDistrict("Wilanów", ["Błonia Wilanowskie", "Kępa Zawadowska", "Miasteczko Wilanów", "Powsin", "Powsinek", "Wilanów Królewski", "Wilanów Niski", "Wilanów Wysoki", "Zawady"]),
  ...inDistrict("Włochy", ["Nowe Włochy", "Okęcie", "Opacz Wielka", "Paluch", "Raków", "Salomea", "Stare Włochy", "Załuski"]),
  ...inDistrict("Wola", ["Czyste", "Koło", "Młynów", "Mirów", "Nowolipki", "Odolany", "Powązki", "Ulrychów"]),
  ...inDistrict("Żoliborz", ["Marymont-Potok", "Sady Żoliborskie", "Stary Żoliborz", "Żoliborz Artystyczny"]),
...
export function canonicalWarsawDistrict(value?: string | null) {
  ...
  const directDistrict = warsawDistrictNames.find((district) => padded.includes(` ${normalizeLocation(district)} `));
...
export function inferWarsawNeighborhood(text?: string | null, district?: string | null) {
  ...
  const matches = [...byExactLabel.entries()]
    .filter(([label, neighborhood]) => districtMatches(neighborhood.district, district) && normalizedText.includes(` ${label} `))
...
function districtMatches(expected: string, actual?: string | null) {
  return !actual || actual === "Bez dzielnicy" || normalizeLocation(expected) === normalizeLocation(actual);
}
