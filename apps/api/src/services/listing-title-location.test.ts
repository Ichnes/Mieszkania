...
test("does not treat a city or district suffix as a street", () => {
  assert.equal(extractStreetFromLocationTitle("Mieszkanie, Warszawa", "Targówek", "Warszawa"), undefined);
  assert.equal(extractStreetFromLocationTitle("Mieszkanie: Targówek", "Targówek", "Warszawa"), undefined);
});
