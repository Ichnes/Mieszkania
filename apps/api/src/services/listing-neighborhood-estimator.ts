...
export async function backfillEstimatedWarsawNeighborhoods(options?: { dryRun?: boolean }) {
  ...
  const references = toReferences(result.rows.map((row) => ({ ...row, district: canonicalWarsawDistrict(row.district) ?? row.district })));
  const changes = result.rows.flatMap((row) => {
    ...
    return neighborhood || district !== row.district ? [{ id: row.id, district, neighborhood: neighborhood ?? existingNeighborhood ?? null }] : [];
  });
...
export function estimateWarsawNeighborhood(
  ...
  const candidates = references.filter((reference) => {
    ...
    return Boolean(canonical && (!districtKey || normalizeLocation(reference.district) === districtKey));
  });

  const streetKey = normalizeStreetName(input.street ?? input.addressText?.split(",", 1)[0]);
  if (streetKey) {
    const sameStreet = candidates.filter((reference) => normalizeStreetName(reference.addressText?.split(",", 1)[0]) === streetKey);
  ...
  const nearby = candidates
    .filter((reference) => reference.latitude != null && reference.longitude != null && input.latitude != null && input.longitude != null)
    .map((reference) => ({
      ...reference,
      distance: distanceMeters(input.latitude!, input.longitude!, reference.latitude!, reference.longitude!)
    }))
...
function toReferences(rows: Array<{
  id: string;
  district: string | null;
  neighborhood: string | null;
  address_text: string | null;
...
function chooseEstimate(references: NeighborhoodReference[], latitude?: number | null, longitude?: number | null) {
  if (references.length === 0) return undefined;
  if (latitude != null && longitude != null) {
    const closest = references
      .filter((reference) => reference.latitude != null && reference.longitude != null)
      .map((reference) => ({ ...reference, distance: distanceMeters(latitude, longitude, reference.latitude!, reference.longitude!) }))
      .sort((left, right) => left.distance - right.distance)[0];
    if (closest) return closest.neighborhood;
  }
  return chooseWeightedNeighborhood(references.map((reference) => ({ ...reference, distance: 500 })));
}

function chooseWeightedNeighborhood(references: Array<NeighborhoodReference & { distance: number }>) {
  const scores = new Map<string, number>();
  for (const reference of references) {
    scores.set(reference.neighborhood, (scores.get(reference.neighborhood) ?? 0) + 1 / Math.max(80, reference.distance));
  }
  return [...scores.entries()].sort((left, right) => right[1] - left[1])[0]?.[0];
}

function normalizeLocation(value?: string | null) {
  return normalizePolish(value ?? "").replace(/[^a-z0-9]+/g, " ").trim();
}
