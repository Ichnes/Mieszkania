import type {
  ImmediateSurroundingsAnalysis,
  ImmediateSurroundingsFinding,
...
const rules: FindingRule[] = [
  ...
  },
  {
    ...
    matches: (tags) =>
      pick(tags, [
        ...
        ["man_made", ["works", "storage_tank", "silo"]],
        ["industrial", null],
      ]),
  },
  {
    category: "construction",
    label: "Budowa / teren poprzemysłowy",
    severity: "attention",
    matches: (tags) =>
      pick(tags, [
  ...
  {
    category: "power",
    label: "Infrastruktura energetyczna",
    severity: "information",
    matches: (tags) => pick(tags, [["power", ["plant", "substation", "generator", "transformer"]]]),
  },
  {
    category: "fuel",
    label: "Stacja paliw",
    severity: "information",
    matches: (tags) => pick(tags, [["amenity", ["fuel"]]]),
  },
  {
    category: "railway",
    label: "Tory kolejowe / tramwajowe",
    severity: "information",
    matches: (tags) => pick(tags, [["railway", ["rail", "light_rail", "tram", "narrow_gauge"]]]),
  },
  {
    category: "major_road",
    label: "Droga o dużym ruchu",
    severity: "information",
    matches: (tags) =>
      pick(tags, [
        [
  ...
  },
  {
    ...
    label: "Lokal nocny",
    severity: "information",
    matches: (tags) => pick(tags, [["amenity", ["nightclub", "bar"]]]),
  },
];

export async function getImmediateSurroundings(
  latitude: number,
  longitude: number,
): Promise<ImmediateSurroundingsAnalysis> {
  ...
  try {
    ...
    url.searchParams.set(
      "bbox",
      buildBoundingBox(latitude, longitude, FETCH_MARGIN_METERS).join(","),
    );
    const payload = await fetchExternalJson<{ elements?: OsmMapElement[] }>(url, {
      headers: {
        Accept: "application/json",
        "User-Agent": "mieszkania-local/0.1 (immediate parcel surroundings)",
      },
      timeoutMs: 12_000,
    });
...
export function analyzeImmediateSurroundings(
  elements: OsmMapElement[],
  latitude: number,
  longitude: number,
  ...
  const unique = [
    ...new Map(
      findings
        ...
        .map((finding) => [finding.osmKey, finding]),
    ).values(),
  ];
  return {
    ...
    assessment: unique.some((finding) => finding.severity === "attention")
      ? ("attention" as const)
      : ("clear" as const),
    findings: unique,
    mapUrl,
...
function project(lon: number, lat: number, originLat: number, originLon: number): [number, number] {
  return [
    (lon - originLon) * 111_320 * Math.cos((originLat * Math.PI) / 180),
    (lat - originLat) * 110_540,
  ];
}
