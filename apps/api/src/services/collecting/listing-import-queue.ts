...
export async function getKnownListingExternalIds(input: {
  sourceKey: string;
  externalIds: string[];
}) {
  ...
  return withDb(async (db) => {
    const result = await db.query<{ external_id: string }>(
      ...
      [
        input.sourceKey,
        input.externalIds,
        input.maximumImageCount,
        input.requiredImageUrlFragment ?? null,
      ],
    );
    return new Set(result.rows.map((row) => row.external_id));
  });
...
export async function releaseDelayedListingImportsNow(input: {
  sourceKey: string;
  limit?: number;
}) {
  return withDb(async (db) => {
...
export async function resetProcessingListingImportsNow(input: {
  sourceKey: string;
  limit?: number;
}) {
  return withDb(async (db) => {
  ...
  for (let attempt = 1; attempt <= 4; attempt += 1) {
    try {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, 75 * attempt + Math.floor(Math.random() * 75)),
      );
    }
  }
