...
export async function sanitizeBrokenPortalListings() {
  return withDb(async (db) => {
    ...
    await insertRemovedEvents(db, [
      ...brokenResult.rows,
      ...missingPriceResult.rows,
      ...unavailableResult.rows,
    ]);

    return {
      removedListings:
        brokenResult.rows.length + missingPriceResult.rows.length + unavailableResult.rows.length,
    };
  });
}
