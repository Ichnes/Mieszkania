...
function buildListingAlerts(row: AlertRow): AlertSummary[] {
  ...
  if ((decisionStage === "to_call" || decisionStage === "new") && !lastContactAt && contactStatus !== "contacted") {
    alerts.push(makeAlert(row, "missing-contact", "medium", "Do pierwszego kontaktu", "Oferta jest w procesie, ale nie ma jeszcze zapisanego kontaktu ani rozmowy."));
  }

  if (decisionStage && decisionStage !== "rejected" && decisionStage !== "bought" && lastContactAt && daysSince(lastContactAt) >= 5) {
    alerts.push(makeAlert(row, "follow-up", "medium", "Brak follow-upu", `Minelo ${daysSince(lastContactAt)} dni od ostatniego kontaktu. Warto odswiezyc rozmowe.`));
  }

  if (scheduledAt && row.viewing_status === "scheduled") {
    const hours = hoursUntil(scheduledAt);
    if (hours >= 0 && hours <= 30) {
      alerts.push(makeAlert(row, "viewing-soon", "high", "Ogladanie juz zaraz", `Masz zaplanowane ogledziny ${formatRelativeHours(hours)}.`));
    }
  }

  if (typeof negotiated === "number" && typeof askingOverride === "number" && askingOverride <= negotiated) {
    alerts.push(makeAlert(row, "target-reached", "high", "Cena w Waszym celu", `Cena po rozmowie ${formatCurrency(askingOverride)} miesci sie w celu negocjacji ${formatCurrency(negotiated)}.`));
  }

  if (decisionStage === "after_viewing" && contactStatus !== "negotiating" && contactStatus !== "closed") {
    alerts.push(makeAlert(row, "after-viewing", "low", "Decyzja po ogladaniu", "Oferta jest po ogladaniu, ale nie ma jeszcze kolejnego kroku: negocjacji, oferty albo odrzucenia."));
  }
...
function normalizeDecisionStage(value?: string | null): ListingDecisionStage | undefined {
  ...
  return value && allowed.includes(value as ListingDecisionStage) ? (value as ListingDecisionStage) : undefined;
}
...
function normalizeContactStatus(value?: string | null): ListingContactStatus | undefined {
  ...
  return value && allowed.includes(value as ListingContactStatus) ? (value as ListingContactStatus) : undefined;
}
