...
export async function getFamilySettings(): Promise<FamilySettings> {
  return withDb(async (db) => {
    ...
    const stored = result.rows[0]?.value ?? (existsSync(localPath) ? JSON.parse(readFileSync(localPath, "utf8")) as FamilySettings : undefined);
...
function validateAndNormalizeSettings(input: FamilySettings): FamilySettings {
  ...
  const normalized: FamilySettings = {
    workplaces: input.workplaces.slice(0, 6).map((workplace, index) => normalizeKnownWorkplace({
...
function normalizeSearchContract(
  ...
  const maxPrice = Math.max(minPrice, Number(stored?.maxPrice ?? defaults.maxPrice) || defaults.maxPrice);
  const minArea = Math.max(0, Number(stored?.minArea ?? defaults.minArea) || defaults.minArea);
  const roomsMin = Math.max(1, Math.floor(Number(stored?.roomsMin ?? defaults.roomsMin) || defaults.roomsMin));
...
export function normalizeDreamProfile(
  ...
  const preferredDistricts = [...new Set(
    (stored?.preferredDistricts ?? defaults.preferredDistricts)
      .map((value) => value.trim())
      .filter(Boolean)
  )].slice(0, 50);
  const minArea = Math.max(0, Number(stored?.minArea ?? defaults.minArea) || defaults.minArea);
  const maxArea = Math.max(minArea, Number(stored?.maxArea ?? defaults.maxArea) || defaults.maxArea);
  const minRooms = Math.max(0, Math.floor(Number(stored?.minRooms ?? defaults.minRooms) || defaults.minRooms));
  const maxPrice = Math.max(0, Number(stored?.maxPrice ?? defaults.maxPrice) || defaults.maxPrice);
  const maxPricePerSqm = Math.max(0, Number(stored?.maxPricePerSqm ?? defaults.maxPricePerSqm) || defaults.maxPricePerSqm);
  const maxMetroDistanceMeters = Math.max(
    0,
    Number(stored?.maxMetroDistanceMeters ?? defaults.maxMetroDistanceMeters) || defaults.maxMetroDistanceMeters
  );
...
async function geocodeWorkplaces(settings: FamilySettings, force = false) {
  const workplaces = await Promise.all(
    settings.workplaces.map(async (workplace) => {
      ...
      return geocoded
        ? normalizeKnownWorkplace({ ...workplace, latitude: geocoded.latitude, longitude: geocoded.longitude })
...
async function ensureWorkplaceCoordinates(settings: FamilySettings) {
  if (!hasMissingWorkplaceCoordinates(settings.workplaces) && !hasSuspiciousWorkplaceCoordinates(settings.workplaces)) {
    return settings;
  }
