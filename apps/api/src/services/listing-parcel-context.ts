...
import { fetchExternalText } from "./external-json";
...
export async function getListingParcelContext(listingId: string, force = false): Promise<ParcelContextResponse | null> {
  ...
  if (!force) {
    const cached = await readCache(listingId, latitude, longitude, false)
  ...
  try {
    ...
    const stale = await readCache(listingId, latitude, longitude, true)
...
export function parcelGeometryContainsPoint(geometry: ParcelGeometry, latitude: number, longitude: number) {
  ...
  for (const endpoint of ULDK_URLS) {
    try {
      ...
      const body = await fetchExternalText(url, {
        headers: { accept: "text/plain", "user-agent": "mieszkania-local/0.1 (ULDK parcel lookup)" },
        timeoutMs: 20_000,
...
export function parseUldkParcelResponse(body: string) {
  ...
  const [id, parcelNumber, commune, region, datasource, ...geometryParts] = lines.slice(1).join("\n").split("|");
...
export function parseWktGeometry(value: string): ParcelGeometry {
  ...
  if (/^POLYGON\s*/i.test(normalized)) {
    return { type: "Polygon", coordinates: parsePolygonText(normalized.replace(/^POLYGON\s*/i, "")) };
  }
...
async function readCache(listingId: string, latitude: number, longitude: number, allowExpired: boolean) {
...
async function readContainingParcelCache(latitude: number, longitude: number, allowExpired: boolean) {
  return withDb(async (db) => {
    const result = await db.query<CachedParcelRow>(`
      select latitude::text, longitude::text, parcel_id, parcel_number, commune, region, datasource, geometry_geojson, fetched_at
      from listing_parcel_context
    ...
    return result.rows.find((row) => parcelGeometryContainsPoint(row.geometry_geojson, latitude, longitude)) ?? null;
  });
}
...
async function saveCache(
  ...
  await withDb((db) => db.query(`
    ...
    on conflict (listing_id) do update set
      ...
      fetched_at = now()
  `, [listingId, latitude, longitude, parcel.id, parcel.number ?? null, parcel.commune ?? null, parcel.region ?? null, parcel.datasource ?? null, JSON.stringify(parcel.geometry)]));
}
