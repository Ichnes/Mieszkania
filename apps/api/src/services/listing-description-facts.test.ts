...
import { enrichListingFromDescription, inferConstructionYear, inferStructuredConstructionYear } from "./listing-description-facts";
...
test("infers construction years from common Polish listing phrases", () => {
  ...
  assert.equal(inferConstructionYear("Apartament w inwestycji Central House z 2022 roku.", 2026), 2022);
  assert.equal(inferConstructionYear("Budynek 2007 roku budowy.", 2026), 2007);
  assert.equal(inferConstructionYear("Budynek został oddany do użytkowania w 2015 roku.", 2026), 2015);
  assert.equal(inferConstructionYear("Willa z 1928 roku, odbudowana w 2008 roku.", 2026), 1928);
  assert.equal(inferConstructionYear("Planowany termin zakończenia inwestycji to 30.03.2026 roku.", 2026), 2026);
});

test("does not mistake unrelated dates for a construction year", () => {
  assert.equal(inferConstructionYear("Mieszkanie po generalnym remoncie z 2020 roku.", 2026), undefined);
  assert.equal(inferConstructionYear("Nowe okna z 2025 roku, budynek przedwojenny.", 2026), undefined);
  assert.equal(inferConstructionYear("Oferta opublikowana w 2024 roku.", 2026), undefined);
  assert.equal(inferConstructionYear("Obok stacja metra oddana do użytku w 2026 roku.", 2026), undefined);
...
test("reads a valid structured construction year and ignores portal zero placeholders", () => {
  assert.equal(inferStructuredConstructionYear({ yearBuilt: 0, nested: { construction_year: "2018" } }, 2026), 2018);
