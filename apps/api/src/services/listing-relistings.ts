...
function scoreRelisting(previous: PreparedRelistingRow, current: PreparedRelistingRow): RelistedListingMatch | null {
  ...
  if (new Date(previous.firstSeenAt).getTime() >= new Date(current.firstSeenAt).getTime()) return null;
  ...
  const exactPrefix = Boolean(previous.descriptionPrefix && previous.descriptionPrefix === current.descriptionPrefix);
  const descriptionSimilarity = computeDescriptionSimilarity(previous.descriptionTokens, current.descriptionTokens);
  ...
  const sameFloor = previous.floor !== null && current.floor !== null && previous.floor === current.floor;
  ...
  const strongSimilarityMatch = descriptionSimilarity >= descriptionSimilarityThreshold
    && areaDifference <= 1
    && sameStreet
  ...
  if (areaDifference <= 0.5) { confidenceScore += 8; reasons.push("ten sam metraż"); }
  else if (areaDifference <= 1) { confidenceScore += 6; reasons.push("niemal ten sam metraż"); }
  else { confidenceScore += 3; reasons.push("podobny metraż"); }
  if (sameAddress) { confidenceScore += 8; reasons.push("ten sam adres"); }
  else if (sameStreet) { confidenceScore += 4; reasons.push("ta sama ulica"); }
  if (sameFloor) { confidenceScore += 4; reasons.push("to samo piętro"); }
  if (nearbyCoordinates) { confidenceScore += 5; reasons.push("ten sam punkt na mapie"); }
  confidenceScore = Math.min(100, confidenceScore);

  const previousPrice = previous.priceAmount ?? undefined;
  const currentPrice = current.priceAmount ?? undefined;
  const priceDifferenceAmount = previousPrice !== undefined && currentPrice !== undefined
    ? currentPrice - previousPrice
    : undefined;
  const priceDifferencePercent = priceDifferenceAmount !== undefined && previousPrice && previousPrice > 0
...
function prepareRow(row: RelistingMatchRow): PreparedRelistingRow {
  ...
  return {
    ...
    descriptionPrefix: prefixTokens.length >= descriptionPrefixWords
...
function tokenizeDescription(value?: string | null) {
  const stopWords = new Set([
    "oraz", "jest", "jako", "przy", "ktore", "ktory", "ktora", "mieszkanie",
    "sprzedaz", "oferta", "nieruchomosci", "warszawa", "lokalizacja", "pokojowe", "pokojowy"
  ]);
...
function areCoordinatesNearby(left: PreparedRelistingRow, right: PreparedRelistingRow) {
  return left.latitude !== null && left.longitude !== null && right.latitude !== null && right.longitude !== null
