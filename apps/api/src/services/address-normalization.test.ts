...
import { estimateWarsawNeighborhood } from "./listing-neighborhood-estimator";
...
test("cuts listing prose without damaging multi-word street names", () => {
  ...
  assert.equal(normalizeWarsawStreetCandidate("Komisji Edukacji Narodowej"), "Komisji Edukacji Narodowej");
});

test("cuts prose that starts directly after a street name", () => {
  assert.equal(
    sanitizeStreetCandidate("Umińskiego położone jest na Gocławiu"),
    "Umińskiego"
  );
});
...
test("accepts only known Warsaw neighborhoods and understands inflected names", () => {
  ...
  assert.equal(inferWarsawNeighborhood("Mieszkanie położone jest na Gocławiu", "Praga-Południe"), "Gocław");
  assert.equal(inferWarsawNeighborhood("Mieszkanie położone jest na Gocławiu", "Mokotów"), undefined);
});
...
test("estimates a neighborhood from the same street and the nearest confirmed point", () => {
  const references = [
    { id: "north", district: "Śródmieście", neighborhood: "Śródmieście Północne", addressText: "Nowogrodzka, Warszawa", latitude: 52.232, longitude: 21.012 },
    { id: "south", district: "Śródmieście", neighborhood: "Śródmieście Południowe", addressText: "Nowogrodzka, Warszawa", latitude: 52.225, longitude: 21.006 }
  ];
  assert.equal(estimateWarsawNeighborhood({ district: "Śródmieście", street: "Nowogrodzka", latitude: 52.2252, longitude: 21.0062 }, references), "Śródmieście Południowe");
});

test("prefers a confirmed street location over a neighborhood mentioned in marketing copy", () => {
  const references = [
    { id: "street", district: "Praga-Południe", neighborhood: "Grochów", addressText: "Grochowska, Warszawa", latitude: 52.244, longitude: 21.084 }
  ];
  assert.equal(
...
test("recovers Warsaw when a portal puts listing prose in the city field", () => {
  assert.equal(
    normalizeWarsawListingCity(
      "Mieszkanie znajduje się przy ul. Cybernetyki",
      "Mokotów"
    ),
    "Warszawa"
  );
