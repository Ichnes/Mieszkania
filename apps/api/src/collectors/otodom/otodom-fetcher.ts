...
export class OtodomFetcher implements ListingFetcher {
  async fetchListing(url: string, options?: { timeoutMs?: number }): Promise<FetchedListingDocument> {
    const timeoutMs = options?.timeoutMs ?? 30_000;
    let lastError: unknown;
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      try {
        const response = await fetch(url, { signal: AbortSignal.timeout(timeoutMs), headers: OTODOM_HEADERS });
        return { url, html: await response.text(), statusCode: response.status, finalUrl: response.url };
      } catch (error) {
        lastError = error;
        if (isCertificateError(error) || (error instanceof TypeError && error.message === "fetch failed")) {
          try {
...
function isCertificateError(error: unknown) {
  ...
  return code === "UNABLE_TO_VERIFY_LEAF_SIGNATURE" || code === "SELF_SIGNED_CERT_IN_CHAIN" || code === "CERT_HAS_EXPIRED";
}

function fetchTrustedOtodomPage(url: string, timeoutMs: number, redirectsLeft = 4): Promise<FetchedListingDocument> {
    ...
    const request = httpsRequest(url, {
      ...
      if (location && statusCode >= 300 && statusCode < 400 && redirectsLeft > 0) {
        ...
        if (!isOtodomHost(redirectedUrl.hostname)) {
          rejectResponse(new Error(`Refusing untrusted Otodom redirect to ${redirectedUrl.hostname}`));
          return;
        }
        void fetchTrustedOtodomPage(redirectedUrl.toString(), timeoutMs, redirectsLeft - 1).then(resolveResponse, rejectResponse);
        return;
      }
