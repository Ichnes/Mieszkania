...
import { claimListingImportBatch, completeListingImport, enqueueListingImports, failListingImport, getKnownListingExternalIds, getListingImportQueueStatus, retryFailedListingImportsNow } from "../../services/listing-import-queue";
...
export class AdresowoCollector {
  ...
  async discoverAll(input: { city: string; startPage?: number; maxPages?: number; batchPages?: number; priority?: number }) {
    ...
    let queued = 0, discovered = 0, scannedPages = 0, currentPage = startPage;
    ...
    while (scannedPages < maxPages) {
      ...
      try {
        ...
        queued += result.queued; discovered += links.length; scannedPages += pages; currentPage += pages;
        if (links.length === 0) break;
      } catch (cause) { error = cause instanceof Error ? cause.message : "Adresowo discovery failed"; break; }
    }
  ...
  async processQueue(input?: { limit?: number; concurrency?: number; queueKind?: "all" | "price_updates" }) {
    ...
    const known = await getKnownListingExternalIds({ sourceKey, externalIds: claimed.map((item) => item.external_id) });
    ...
    for (let offset = 0; offset < claimed.length; offset += concurrency) {
      ...
      const results = await Promise.allSettled(batch.map((item) => {
        ...
        return this.collect(item.canonical_url, item.external_id, { refreshMode: priceOnly ? "price_only" : "full", downloadMedia: !priceOnly });
      }));
      for (let index = 0; index < results.length; index += 1) {
        const result = results[index], item = batch[index];
        if (result.status === "fulfilled") { completed += 1; await completeListingImport(item.id); }
        else { failed += 1; const error = result.reason instanceof Error ? result.reason.message : "unknown error"; failures.push({ externalId: item.external_id, url: item.canonical_url, error }); await failListingImport({ id: item.id, error, attempts: item.attempts + 1 }); }
      }
    }
  ...
  private async discover(city: string, startPage: number, pages: number) {
    ...
    for (let offset = 0; offset < pages; offset += 1) {
      ...
      for (const match of html.matchAll(/href=["'](\/o\/mieszkanie-[^"'?#]+-([a-z]\d[a-z]\d[a-z]\d))["']/gi)) {
  ...
  private async collect(url: string, externalId: string, options?: { refreshMode?: "full" | "price_only"; downloadMedia?: boolean }) {
    ...
    if (!parsed.priceAmount && parsed.status !== "removed") throw new Error(`MISSING_PRICE: ${externalId} (${url})`);
    ...
    const archived = full
      ? await archiveOfferArtifacts({ sourceKey, externalId: parsed.externalId, timestamp: new Date().toISOString(), html: page.html, parsed })
      : null;
...
function fetchTrustedAdresowoPage(url: string, redirectsLeft = 4): Promise<string> {
  ...
  if (target.hostname !== "adresowo.pl" && !target.hostname.endsWith(".adresowo.pl")) {
    return Promise.reject(new Error(`Refusing TLS fallback for untrusted Adresowo host ${target.hostname}`));
  }
  return new Promise((resolvePage, rejectPage) => {
    const request = httpsRequest(url, { method: "GET", headers: requestHeaders, rejectUnauthorized: false, timeout: 45_000 }, (response) => {
      const status = response.statusCode ?? 500;
      const location = response.headers.location;
      if (location && status >= 300 && status < 400 && redirectsLeft > 0) {
        response.resume();
        const redirectedUrl = new URL(location, url);
        if (redirectedUrl.hostname !== "adresowo.pl" && !redirectedUrl.hostname.endsWith(".adresowo.pl")) {
          rejectPage(new Error(`Refusing untrusted Adresowo redirect to ${redirectedUrl.hostname}`));
          return;
        }
        void fetchTrustedAdresowoPage(redirectedUrl.toString(), redirectsLeft - 1).then(resolvePage, rejectPage);
        return;
      }
    ...
    request.on("timeout", () => request.destroy(new Error(`Adresowo request timed out for ${url}`)));
    request.on("error", rejectPage);
    request.end();
  });
}

async function fetchStaticPage(url: string) { const html = await fetchHtml(url); return { html, text: strip(html) }; }
...
async function fetchRenderedPage(url: string) {
  ...
  try {
    ...
    for (const name of ["Tylko niezbędne", "Akceptuj"]) {
      ...
      if (await button.count()) { await button.click({ timeout: 1_500 }).catch(() => undefined); break; }
    }
    // The initial page carries only the cover. Opening this exact showcase
    // renders all slides belonging to the current offer.
    const gallery = page.locator("button[data-track='offer-gallery-more-images'], button[data-track='offer-gallery-show-all'], button[onclick*='showcase.image.show']").first();
    if (await gallery.count()) {
      await gallery.click({ timeout: 4_000, force: true }).catch(() => undefined);
      await page.locator(".swiper-container-showcase .swiper-slide img").first().waitFor({ state: "attached", timeout: 4_000 }).catch(() => undefined);
      await page.waitForTimeout(350);
    }
    const description = page.locator("#showMore").first();
    if (await description.count()) { await description.click({ timeout: 4_000, force: true }).catch(() => undefined); await page.waitForTimeout(150); }
...
function parseListing(url: string, html: string, visibleText: string, externalId: string): ParsedListing {
  ...
  const extractedDescription = strip(html.match(/<p[^>]+id=["']description["'][^>]*>([\s\S]*?)<\/p>/i)?.[1] ?? string(place?.description) ?? "");
  const title = strip(html.match(/<h1[^>]*>([\s\S]*?)<\/h1>/i)?.[1] ?? meta(html, "og:title") ?? "Oferta Adresowo");
  const isOwnerListing = /bezpośrednio\s+od\s+właściciela|bez\s+pośrednik(?:a|ów)|sprzedaż\s+bezpośrednia/i.test(`${title} ${extractedDescription} ${visibleText}`);
  ...
  const description = isOwnerListing && !/bezpośrednio\s+od\s+właściciela/i.test(extractedDescription)
  ...
  const area = amount(title.match(/([\d,.]+)\s*m(?:²|2)/i)?.[1] ?? text.match(/powierzchni\s+([\d,.]+)\s*m(?:²|2)/i)?.[1]);
  ...
  const district = text.match(/(?:Mokotów|Wilanów|Wola|Ursynów|Żoliborz|Bemowo|Ochota|Bielany|Włochy|Praga-Północ|Praga-Południe|Śródmieście|Targówek|Białołęka|Wawer)/i)?.[0];
  const phone = visibleText.match(/\b(?:\+48\s*)?(\d{3}[\s-]?\d{3}[\s-]?\d{3})\b/)?.[1]?.replace(/[\s-]/g, "");
  const images = extractShowcaseImages(html);
  const normalizedImages = Array.from(new Map(images.map((value) => [value.replace(/_xbig(?=-)/i, ""), value])).values());
  return { externalId, canonicalUrl: url, title, description: description || undefined, city: "Warszawa", district, addressText: address, latitude: amount(string(geo?.latitude)), longitude: amount(string(geo?.longitude)), sourceContactPhone: phone, priceAmount: amount(string(offer?.price) ?? text.match(/([\d\s.]+)\s*zł/i)?.[1]), areaSqm: area, rooms, floor, yearBuilt, publishedAt, marketType: /deweloper|rynek pierwotny/i.test(text) ? "primary" : "secondary", offerType: "sale", status: /ogłoszenie (?:jest )?nieaktualne|oferta nieaktualna/i.test(text) ? "removed" : "active", images: normalizedImages.map((sourceUrl, position) => ({ sourceUrl, position, isPrimary: position === 0 })), rawPayload: { url, jsonLd: json, yearBuilt, advertiserType: isOwnerListing ? "private" : undefined } };
}
...
export function extractAdresowoConstructionYear(html: string, visibleText = "") {
  ...
  const fromAttributeBox = html.match(/box__attributes--content[\s\S]{0,500}?Rok\s+budowy\s*:[\s\S]{0,220}?class=["'][^"']*fsize-a[^"']*["'][^>]*>\s*((?:18|19|20)\d{2})\s*</i)?.[1];
  ...
  return Number.isInteger(parsed) && parsed >= 1800 && parsed <= new Date().getFullYear() + 5 ? parsed : undefined;
}

function extractShowcaseImages(html: string) {
  const showcaseStart = html.search(/<div[^>]*class=["'][^"']*\bswiper-container-showcase\b[^"']*["'][^>]*>/i);
  const similarOffersStart = html.search(/<div[^>]*id=["']similar-offers["'][^>]*>/i);
  const paginationOffset = showcaseStart >= 0
    ? html.slice(showcaseStart).search(/<div[^>]*class=["'][^"']*\bswiper-pagination\b[^"']*["'][^>]*>/i)
    : -1;
  const paginationStart = paginationOffset > 0 ? showcaseStart + paginationOffset : -1;
  const safeEnds = [paginationStart, similarOffersStart].filter((position) => position > showcaseStart);
  ...
  const showcaseHtml = showcaseStart >= 0 && showcaseEnd > showcaseStart ? html.slice(showcaseStart, showcaseEnd) : "";
  const galleryImages = Array.from(showcaseHtml.matchAll(/https:\/\/s\d+\.adresowa\.pl\/oi\/[^"'\s<>]+\.(?:jpg|jpeg|webp|png)/gi), (match) => match[0]);
...
function extractRelativePublishedAt(value: string) {
  ...
  const match = normalized.match(/dodana\s+(?:ponad\s+)?(?:(\d+)\s+)?(minut\w*|godzin\w*|dni?\w*|tygodni\w*|miesiac\w*|lat\w*)\s+temu/);
  if (!match) return undefined;
  const quantity = Number(match[1] ?? 1);
  if (!Number.isFinite(quantity) || quantity <= 0) return undefined;
  const unit = match[2];
  const milliseconds = unit.startsWith("minut") ? quantity * 60_000
    : unit.startsWith("godzin") ? quantity * 3_600_000
      : /^(?:dzien|dni)/.test(unit) ? quantity * 86_400_000
        : unit.startsWith("tygod") ? quantity * 7 * 86_400_000
          // Adresowo gives relative, rounded text. For ranking, treat a month
          // as four weeks from the moment this offer was fetched.
          : unit.startsWith("miesiac") ? quantity * 28 * 86_400_000
            : quantity * 365 * 86_400_000;
  return new Date(Date.now() - milliseconds).toISOString();
}

function jsonLd(html: string) { return Array.from(html.matchAll(/<script[^>]+type=["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/gi)).flatMap((match) => { try { const value = JSON.parse(match[1]) as Record<string, unknown>; const graph = value["@graph"]; return Array.isArray(graph) ? graph.filter((entry): entry is Record<string, unknown> => Boolean(entry) && typeof entry === "object") : [value]; } catch { return []; } }); }
function typeIncludes(value: Record<string, unknown>, type: string) { const candidate = value["@type"]; return Array.isArray(candidate) ? candidate.includes(type) : candidate === type; }
...
function string(value: unknown) { return typeof value === "string" ? value : typeof value === "number" ? String(value) : undefined; }
...
function strip(value: string) { return value.replace(/<br\s*\/?>/gi, "\n").replace(/<[^>]+>/g, " ").replace(/&nbsp;/gi, " ").replace(/&amp;/gi, "&").replace(/\s+/g, " ").trim(); }
function delay(milliseconds: number) { return new Promise<void>((resolve) => setTimeout(resolve, milliseconds)); }
const requestHeaders = { accept: "text/html,application/xhtml+xml", "accept-language": "pl-PL,pl;q=0.9", "user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/126 Safari/537.36" };
