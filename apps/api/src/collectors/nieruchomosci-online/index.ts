...
} from "../../services/listings/listing-description-facts";
import {
  extractStreetFromLocationTitle,
  inferWarsawDistrictFromLocationTitle,
  inferWarsawDistrictFromText,
} from "../../services/listings/listing-title-location";
...
export class NieruchomosciOnlineCollector {
  ...
  async processQueue(input?: {
    ...
    if (input?.force) {
      ...
      await releaseDelayedListingImportsNow({
        sourceKey,
    ...
    while (claimedCount < limit && this.rateLimitedUntil <= Date.now()) {
      const claimed = await claimListingImportBatch({
        sourceKey,
        limit: Math.min(15, limit - claimedCount),
      ...
      const worker = async () => {
        while (cursor < claimed.length) {
          ...
          try {
            ...
            await this.collectOneInternal(item.canonical_url, item.external_id, {
  ...
  private async collectOneInternal(
    url: string,
    queuedExternalId: string,
    options?: { refreshMode?: "full" | "price_only"; downloadMedia?: boolean },
  ) {
    ...
    try {
      ...
      if (
        isPriceOnlyRefresh &&
        !parsed.priceAmount &&
        !isUnavailableNieruchomosciOnlineListing(rendered.text) &&
        !rendered.usedBrowser
      ) {
        await this.waitForDetailRequestSlot();
        rendered = await fetchRenderedPage(url, ["Rozwiń opis", "Oddzwoń do mnie"], {
          mode: "price_only",
      ...
      return {
        ...stored,
        archived: archived ?? { basePath: "", rawHtmlPath: "", parsedJsonPath: "" },
      };
    } catch (error) {
      if (isRateLimitedError(error)) {
        this.rateLimitedUntil = Math.max(
          this.rateLimitedUntil,
          Date.now() + rateLimitCooldownMinutes * 60_000,
        );
  ...
  private async waitForDetailRequestSlot() {
    const scheduled = this.detailRequestSchedule.then(async () => {
      if (this.rateLimitedUntil > Date.now()) {
        throw new Error(
          `RATE_LIMITED: cooldown active until ${new Date(this.rateLimitedUntil).toISOString()}`,
        );
      }
      ...
      if (this.rateLimitedUntil > Date.now())
        throw new Error("RATE_LIMITED: cooldown activated while waiting");
...
function isUnavailableNieruchomosciOnlineListing(visibleText: string) {
  return /ups\s*[.!…]*\s*to og\u0142oszenie ju\u017c nie istnieje|to og\u0142oszenie jest ju\u017c nieaktualne/i.test(
    visibleText,
  );
}
...
async function fetchHtml(url: string, timeoutMs = 60_000, attempts = 3) {
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      ...
      if (
        ...
        isNieruchomosciOnlineHost(new URL(url).hostname)
      ) {
  ...
  throw lastError instanceof Error
    ? lastError
    : new Error(`Nieruchomosci-online fetch failed: ${url}`);
}
...
function fetchTrustedNieruchomosciOnlinePage(
  url: string,
  redirectsLeft = 4,
  timeoutMs = 60_000,
): Promise<string> {
  return new Promise((resolvePage, rejectPage) => {
    ...
    if (!isNieruchomosciOnlineHost(target.hostname)) {
      rejectPage(
        new Error(`Refusing untrusted Nieruchomosci-online page host: ${target.hostname}`),
      );
      return;
    }
      ...
      (response) => {
        ...
        if (location && status >= 300 && status < 400 && redirectsLeft > 0) {
          ...
          if (!isNieruchomosciOnlineHost(redirectedUrl.hostname)) {
            rejectPage(
              new Error(
                `Refusing untrusted Nieruchomosci-online redirect to ${redirectedUrl.hostname}`,
              ),
            );
            return;
          }
          void fetchTrustedNieruchomosciOnlinePage(
        ...
        if (status < 200 || status >= 300) {
          ...
          rejectPage(
            new Error(
              status === 429 ? `RATE_LIMITED: HTTP 429 (${url})` : `HTTP ${status}: ${url}`,
            ),
          );
          return;
        }
    ...
    request.on("timeout", () =>
      request.destroy(new Error(`Nieruchomosci-online request timed out for ${url}`)),
    );
...
function extractReferences(html: string) {
  return Array.from(
    html.matchAll(/href=["']([^"']*\/mieszkanie,na-sprzedaz\/(\d+)\.html)[^"']*["']/gi),
  ).map((match) => ({
    externalId: `nieruchomosci-online-${match[2]}`,
    url: new URL(match[1], "https://www.nieruchomosci-online.pl").toString(),
  }));
}

function dedupe(items: SourceListingReference[]) {
...
export function parseListing(
  ...
  const nodes = json.flatMap((value) =>
    Array.isArray(value["@graph"]) ? (value["@graph"] as Record<string, unknown>[]) : [value],
  );
  const product =
    nodes.find((node) => node.offers && typeof node.offers === "object") ??
    nodes.find((node) => node.name || node.description) ??
    {};
  ...
  const offer = Array.isArray(offerValue)
    ...
    : typeof offerValue === "object" && offerValue
      ? (offerValue as Record<string, unknown>)
      : {};
  const title = text(product.name) ?? meta(html, "og:title") ?? "Oferta Nieruchomosci-online";
  const description =
    descriptionOverride?.trim() ||
    extractDescription(visibleText) ||
    strip(text(product.description) ?? meta(html, "og:description") ?? "");
  ...
  const priceNearLabel = (value: string) =>
    portalPrice ?? value.match(/\b([0-9]{1,3}(?:[\s\u00a0][0-9]{3})+)\s*(?:z\u0142|PLN)\b/i)?.[1];
  const price = number(
    text(offer.price) ?? priceNearLabel(visibleText) ?? description.match(/([0-9\s]+)\s*zł/i)?.[1],
  );
  const area = number(
    text(product.floorSize) ?? visibleText.match(/([0-9]+(?:[,.][0-9]+)?)\s*m(?:²|2)/i)?.[1],
  );
  ...
  const rooms =
    domiportaRooms ??
    number(
      text(product.numberOfRooms) ??
        visibleText.match(/(?:Liczba pokoi\s*:?\s*|\b)([1-9])[- ]*pokoj/i)?.[1],
    );
  ...
  const district =
    ...
    address.district ??
    inferWarsawDistrictFromText(location);
  const addressStreet =
    address.street && normalizePolish(address.street) !== normalizePolish("Warszawa")
      ? address.street
      : undefined;
  ...
  const yearBuilt =
    inferStructuredConstructionYear(product) ??
  ...
  const isArchived =
    /to og[łl]oszenie jest ju[żz] nieaktualne|og[łl]oszenia archiwalne prezentujemy/i.test(
      `${html} ${visibleText}`,
    );
  return {
    externalId,
    ...
    areaSqm: area,
    rooms,
    floor: portalDetails.floor,
    totalFloors: portalDetails.totalFloors,
    yearBuilt,
    publishedAt: publishedAt ?? undefined,
    marketType: /rynek pierwotny/i.test(location) ? "primary" : "secondary",
    offerType: "sale",
    status: isArchived ? "removed" : "active",
    ...
    rawPayload: {
      url,
      jsonLd: json,
      portalFeatures: { fees: portalDetails.maintenanceFee, yearBuilt: portalDetails.yearBuilt },
    },
  };
}
...
function extractNieruchomosciAddress(html: string) {
  ...
  const values = Array.from(block.matchAll(/<a[^>]*>([\s\S]*?)<\/a>/gi))
  ...
  const plain = strip(block)
    ...
    .filter(Boolean);
  return {
    street: values[0] ?? plain[0],
    district:
      values[1] ??
      plain.find((value) =>
        /Mokotów|Wilanów|Wola|Ursynów|Żoliborz|Bemowo|Ochota|Bielany|Włochy|Praga|Śródmieście/i.test(
          value,
        ),
      ),
  };
...
export async function fetchRenderedPage(
  ...
  try {
    ...
    if (options?.mode !== "price_only") {
      ...
      const nieruchomosciDescription = page
        .locator("div.estate-desc-less > button.show-more-a")
        .first();
      if (await nieruchomosciDescription.count()) {
        await nieruchomosciDescription
          .click({ timeout: 5_000, force: true })
          .catch(() => undefined);
        await page.waitForTimeout(250);
      }

      const nieruchomosciPhone = page
        .locator(
          "p.phone.first .icon-arrow-black-small-right-a, p.phone.first .icon-arrow-black-small-right",
        )
        .first();
      if (await nieruchomosciPhone.count())
        await nieruchomosciPhone.click({ timeout: 2500 }).catch(() => undefined);
...
function priceNearLabel(value: string) {
  return (
    value.match(/(?:Cena|zł\/m²)[\s:]*([0-9\s]+)\s*zł/i)?.[1] ??
    value.match(/\b([0-9]{1,3}(?:[\s\u00a0][0-9]{3})+)\s*zł\b/i)?.[1]
  );
}

function extractDescription(value: string) {
  const match = value.match(
    /Opis (?:nieruchomości|oferty)\s*([\s\S]*?)(?:\n(?:Rozwiń opis|Rozwiń|Zwiń|Kontakt|Zapytaj o ofertę)\b|$)/i,
  );
...
function isListingPhoto(value: string) {
  ...
  return (
    /\.(?:jpg|jpeg|webp|png)(?:[?#].*)?$/i.test(value) &&
...
function extractListingImages(url: string, html: string, primaryImage?: string) {
  ...
  const candidates = hostname.includes("domiporta.pl")
    ? Array.from(
        html.matchAll(
          /<source[^>]+srcset=["'](https:\/\/galeria\.domiporta\.pl\/pictures\/big\/[^"']+\.jpg(?:[?#][^"']*)?)/gi,
        ),
      ).map((match) => match[1])
    : hostname.includes("nieruchomosci-online.pl")
      ? Array.from(
          html.matchAll(/\bdata-image=["'](https?:[^"']+\.(?:jpg|jpeg|webp|png)[^"']*)/gi),
        ).map((match) => match[1])
      : Array.from(
          html.matchAll(/(?:src|data-src)=["'](https?:[^"']+\.(?:jpg|jpeg|webp|png)[^"']*)/gi),
        ).map((match) => match[1]);

  return Array.from(
    new Set(
      [primaryImage, ...candidates]
        .filter((candidate): candidate is string => Boolean(candidate))
        .filter(isListingPhoto),
    ),
  ).slice(0, 40);
}
...
function extractPortalListingDetails(url: string, html: string) {
  ...
  if (hostname.includes("nieruchomosci-online.pl")) {
    const fee = parsePortalAmount(
      html.match(
        /<strong>\s*Czynsz:\s*<\/strong>[\s\S]{0,120}?<span[^>]*>\s*([\d\s&nbsp;]+)\s*z(?:ł|l)/i,
      )?.[1],
    );
...
export function extractNieruchomosciOnlineConstructionYear(html: string) {
  const attributeBoxYear = html.match(
    /box__attributes--content[\s\S]{0,500}?Rok\s+budowy\s*:[\s\S]{0,220}?class=["'][^"']*fsize-a[^"']*["'][^>]*>\s*((?:18|19|20)\d{2})\s*</i,
  )?.[1];
  const legacyYear = html.match(
    /<strong>\s*Rok\s+budowy:\s*<\/strong>[\s\S]{0,160}?<span[^>]*>\s*((?:18|19|20)\d{2})/i,
  )?.[1];
