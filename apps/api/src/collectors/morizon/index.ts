...
export class MorizonCollector {
  ...
  async discoverAll(input: { city: string; startPage?: number; maxPages?: number; batchPages?: number; priority?: number }) {
    ...
    while (scannedPages < maxPages) {
      ...
      try {
        ...
        const result = await enqueueListingImports({ sourceKey, city, priority: input.priority ?? 100, items: references });
    ...
    for (let offset = 0; offset < claimed.length; offset += concurrency) {
      ...
      for (let index = 0; index < results.length; index += 1) {
        ...
        if (result.status === "fulfilled") {
          ...
          failures.push({ externalId: item.external_id, url: item.canonical_url, error: errorMessage });
          await failListingImport({ id: item.id, error: errorMessage, attempts: item.attempts + 1 });
        }
      }
    }
  ...
  private async discover(city: string, startPage: number, pages: number) {
    ...
    return {
      references: Array.from(new Map(references.map((reference) => [reference.externalId, reference])).values()),
      scannedPages,
      reachedEnd
    };
  }
  ...
  private async collect(url: string, externalId: string, options?: { refreshMode?: "full" | "price_only"; downloadMedia?: boolean }) {
    ...
    if (parsed.externalId !== externalId) {
      throw new Error(`MORIZON_ID_MISMATCH: expected ${externalId}, received ${parsed.externalId} (${url})`);
    }
...
export async function fetchRenderedMorizonPage(url: string, options?: { mode?: "full" | "price_only" }) {
  ...
  try {
    ...
    await page.locator("h1").first().waitFor({ state: "attached", timeout: 15_000 }).catch(() => undefined);
    ...
    if (options?.mode !== "price_only") {
      ...
      if (await showPhone.count()) {
        ...
        await page.locator('[data-cy="phoneContactNumber"]').first()
          .waitFor({ state: "attached", timeout: 5_000 }).catch(() => undefined);
      }
      ...
      await galleryOpener.click({ timeout: 10_000, force: true }).catch(async () => {
        await page.locator('button[data-cy="detailsGalleryItemPhoto"]').last().click({ timeout: 5_000, force: true }).catch(() => undefined);
      });
      const firstGalleryItem = page.locator('button.gallery__photos-item[aria-label="Zdjęcie nr 1"], button.gallery__photos-item').first();
      ...
      if (await firstGalleryItem.count()) {
        ...
        await page.locator("swiper-slide img.slider__item-image").first()
          .waitFor({ state: "attached", timeout: 8_000 }).catch(() => undefined);
      }
    ...
    const request = httpsRequest(url, {
      ...
      if (location && status >= 300 && status < 400 && redirectsLeft > 0) {
        ...
        if (!isMorizonPageHost(redirectedUrl.hostname)) {
          rejectPage(new Error(`Refusing untrusted Morizon redirect to ${redirectedUrl.hostname}`));
          return;
        }
        void fetchTrustedMorizonPage(redirectedUrl.toString(), redirectsLeft - 1).then(resolvePage, rejectPage);
        return;
      }
