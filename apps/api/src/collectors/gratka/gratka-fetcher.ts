...
export class GratkaFetcher implements ListingFetcher {
  ...
  async fetchListing(url: string, options?: { includeGallery?: boolean; preferStaticHtml?: boolean }): Promise<FetchedListingDocument> {
    ...
    try {
      ...
      await page.locator(".gallery-more__button, img[data-cy='thumbnail'], [data-cy='phoneContactButton']")
        .first()
      ...
      try {
        ...
        await clickFirstMatchingElement(page, "button[aria-label*='Zamknij'], .gallery__close", 1_000);
        await dismissBlockingOverlays(page);
        await clickFirstMatchingElement(page, "[data-cy='phoneContactButton'], .phone-contact__button", 2_500);
        await page.waitForFunction(
          () => Array.from(document.querySelectorAll(".phone-contact__number"))
            .some((node) => node.textContent && /\d/.test(node.textContent) && !node.textContent.includes("...")),
          undefined,
          { timeout: 2_500 }
        ).catch(() => undefined);
        contactHtml = await page.content();
      } catch (cause) {
        console.warn(`[gratka] Contact rendering failed for ${url}; keeping the captured gallery.`, cause);
      }
      ...
      console.warn(`[gratka] Browser rendering failed for ${url}; falling back to static HTML.`, cause);
      return this.fetchStaticListing(url);
    } finally {
  ...
  private async fetchStaticListing(url: string) {
    ...
    throw lastError instanceof Error ? lastError : new Error(`Gratka static fetch failed for ${url}`);
  }
}
...
async function clickFirstMatchingElement(page: Page, selector: string, timeout: number) {
  ...
  const clickedByLocator = await locator.click({ timeout })
    .then(() => true)
    .catch(() => false);
  ...
  const forceClickedByLocator = await locator.click({ timeout: Math.min(timeout, 2_000), force: true })
    .then(() => true)
    .catch(() => false);
    ...
    const request = httpsRequest(url, {
      ...
      if (location && statusCode >= 300 && statusCode < 400 && redirectsLeft > 0) {
        ...
        if (!isGratkaPageHost(redirectedUrl.hostname)) {
          rejectResponse(new Error(`Refusing untrusted Gratka redirect to ${redirectedUrl.hostname}`));
          return;
        }
        void fetchTrustedGratkaPage(redirectedUrl.toString(), redirectsLeft - 1).then(resolveResponse, rejectResponse);
        return;
      }
...
function isCertificateError(error: unknown) {
  ...
  return /certificate|cert_|unable to verify/i.test(`${error.message} ${cause instanceof Error ? `${cause.message} ${(cause as NodeJS.ErrnoException).code ?? ""}` : ""}`);
}

async function dismissBlockingOverlays(page: Page) {
  const consentButton = page.locator(".cmp-intro_acceptAll, button[aria-label='Przejdź do serwisu']").first();
  if (await consentButton.count().catch(() => 0)) {
    await consentButton.click({ timeout: 4_000, force: true }).catch(() => undefined);
    await page.locator(".cmp-popup_popup").waitFor({ state: "hidden", timeout: 4_000 }).catch(() => undefined);
  }
  ...
  await page.evaluate(() => {
    document.querySelectorAll(".cmp-popup_overlay, .cmp-popup_popup")
      .forEach((node) => node.remove());
  }).catch(() => undefined);
}

async function openFullGallery(page: Page) {
  await page.locator(".gallery-more__button, button.gallery-more__button")
    .first()
  ...
  const clickedFirstPhoto = await clickFirstMatchingElement(
    page,
  ...
  await page.waitForFunction(
    () => {
      ...
      const expandedPhotos = document.querySelectorAll("button.gallery__photos-item img[data-cy='thumbnail']");
  ...
  const slideCount = await page.locator("swiper-slide[aria-label*=' / ']").count().catch(() => 0);
  const photoCount = await page.locator("button.gallery__photos-item img[data-cy='thumbnail']").count().catch(() => 0);
  return { clickedMore, clickedFirstPhoto, slideCount, photoCount };
}

async function collectRenderedGalleryImageUrls(page: Page) {
  return page.evaluate(() => {
    const galleryImages = Array.from(document.querySelectorAll<HTMLImageElement>(
      "swiper-slide[aria-label*=' / '] img, button.gallery__photos-item img[data-cy='thumbnail']"
    ));
    const images = galleryImages.length > 3
      ? galleryImages
      : Array.from(document.querySelectorAll<HTMLImageElement>("img[data-cy='thumbnail'], img"));
