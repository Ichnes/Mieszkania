...
export class OlxCollector {
  ...
  async collectOne(url: string, options?: { downloadMedia?: boolean; externalId?: string; refreshMode?: "full" | "price_only" }) {
    ...
    const normalizedParsed = geocoded ? { ...parsed, latitude: geocoded.latitude, longitude: geocoded.longitude } : parsed;
    const reverseGeocoded =
      !isPriceOnlyRefresh && normalizedParsed.latitude && normalizedParsed.longitude
        ? await reverseGeocodeListing({ latitude: normalizedParsed.latitude, longitude: normalizedParsed.longitude })
        : null;
  ...
  async processQueue(input?: { limit?: number; concurrency?: number; downloadMedia?: boolean; queueKind?: "all" | "price_updates" }) {
    ...
    const claimed = await claimListingImportBatch({ sourceKey: "olx", limit, queueKind: input?.queueKind });
    const knownExternalIds = await getKnownListingExternalIds({ sourceKey: "olx", externalIds: claimed.map((item) => item.external_id) });
    let completed = 0;
    let failed = 0;
      ...
      for (let offset = 0; offset < results.length; offset += 1) {
        ...
        await failListingImport({
          ...
          metadata: message.startsWith("MISSING_PRICE:")
            ? { failureCode: "missing_price", missingPrice: true, externalId: item.external_id, canonicalUrl: item.canonical_url }
            : undefined
        });
      }
    }
