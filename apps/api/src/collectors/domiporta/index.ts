...
export class DomiportaCollector {
  ...
  async collectOne(url: string, options?: { refreshMode?: "full" | "price_only"; downloadMedia?: boolean }) {
    ...
    return this.collect(url, `domiporta-${match?.[1] ?? Buffer.from(url).toString("base64url").slice(0, 16)}`, options);
  }
  ...
  async discoverAll(input: { city: string; startPage?: number; maxPages?: number; batchPages?: number; priority?: number }) {
    ...
    let queued = 0, discovered = 0, scannedPages = 0, currentPage = startPage, emptyBatches = 0;
    let error: string | undefined;
    while (scannedPages < maxPages) {
      try {
        const links = await this.discover(city, currentPage, Math.min(batchPages, maxPages - scannedPages));
        const result = await enqueueListingImports({ sourceKey, city, priority: input.priority ?? 100, items: links });
        queued += result.queued; discovered += links.length; scannedPages += Math.min(batchPages, maxPages - scannedPages); currentPage += batchPages;
      ...
      } catch (cause) { error = cause instanceof Error ? cause.message : "Domiporta discovery failed"; break; }
    }
  ...
  async processQueue(input?: { limit?: number; concurrency?: number; queueKind?: "all" | "price_updates" }) {
    ...
    const concurrency = Math.max(1, Math.min(maxQueueConcurrency, input?.concurrency ?? maxQueueConcurrency));
  ...
  private async discover(city: string, startPage: number, pages: number) {
    ...
    for (let offset = 0; offset < pages; offset += 1) {
      ...
      url.searchParams.set("Price.From", "900000"); url.searchParams.set("Price.To", "2000000"); url.searchParams.set("Rooms.From", "3"); url.searchParams.set("Rynek", "Wtorny");
      if (startPage + offset > 1) {
        const page = String(startPage + offset);
        url.searchParams.set("PageNumber", page);
      }
      const html = await fetchHtml(url.toString());
      for (const match of html.matchAll(/(?:href|data-href)=["']([^"']*\/(?:nieruchomosci|mieszkanie)\/[^"']*\/(\d+)(?:[?#][^"']*)?)["']/gi)) {
        all.push({ externalId: `domiporta-${match[2]}`, url: new URL(match[1], "https://www.domiporta.pl").toString() });
      }
    }
  ...
  private async collect(url: string, externalId: string, options?: { refreshMode?: "full" | "price_only"; downloadMedia?: boolean }) {
    ...
    if (isPriceOnlyRefresh && !parsed.priceAmount) {
      rendered = await fetchRenderedPage(url, ["Rozwiń", "Pokaż telefon", "Zobacz galerię"], { mode: "price_only" });
      parsed = parseListing(url, rendered.html, externalId, rendered.text, rendered.description);
    }
    if (!parsed.priceAmount && parsed.status !== "removed") throw new Error(`MISSING_PRICE: ${parsed.externalId} (${url})`);
    if (!isPriceOnlyRefresh && (!parsed.latitude || !parsed.longitude)) { const point = await geocodeListing({ city: parsed.city, district: parsed.district, neighborhood: parsed.neighborhood, street: parsed.street, addressText: parsed.addressText }); if (point) Object.assign(parsed, point); }
...
async function fetchHtml(url: string) {
  try {
    ...
    if (error instanceof TypeError && error.message === "fetch failed" && isDomiportaHost(hostname)) {
...
function fetchTrustedDomiportaPage(url: string, redirectsLeft = 4): Promise<string> {
  ...
  if (!isDomiportaHost(target.hostname)) {
    return Promise.reject(new Error(`Refusing Domiporta fallback for untrusted host ${target.hostname}`));
  }
    ...
    const request = httpsRequest(target, {
      ...
      if (statusCode >= 300 && statusCode < 400 && location && redirectsLeft > 0) {
        ...
        if (!isDomiportaHost(redirectedUrl.hostname)) {
          reject(new Error(`Refusing Domiporta redirect to untrusted host ${redirectedUrl.hostname}`));
          return;
        }
        void fetchTrustedDomiportaPage(redirectedUrl.toString(), redirectsLeft - 1).then(resolve, reject);
        return;
      }
