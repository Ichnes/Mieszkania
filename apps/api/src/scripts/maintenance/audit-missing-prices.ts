...
function summarize(rows: PriceAuditRow[]) {
  ...
  return {
    ...
    bySource: Object.fromEntries(
      Object.entries(
        rows.reduce<Record<string, { total: number; missing: number }>>((result, row) => {
        ...
        source,
        {
          ...values,
          percent:
            values.total > 0 ? Number(((values.missing / values.total) * 100).toFixed(2)) : 0,
        },
      ]),
    ),
    ...
    JSON.stringify(
      {
        ...
        activeListings,
        recentWarsaw,
        recentMissingSamples: recentMissingSamples.rows,
      },
      null,
      2,
    ),
