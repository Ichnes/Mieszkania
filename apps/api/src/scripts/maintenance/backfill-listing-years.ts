...
async function main() {
  ...
  const inferred = candidates.rows.flatMap((row) => {
    const description =
      row.source_key === "domiporta" ? row.description?.slice(0, 5_000) : row.description;
    const yearBuilt =
      inferStructuredConstructionYear(row.snapshot_payload) ??
      inferConstructionYear([row.title, description].filter(Boolean).join("\n"));
    return yearBuilt ? [{ ...row, yearBuilt }] : [];
  });
  const bySource = Object.entries(
    inferred.reduce<Record<string, number>>((result, row) => {
  ...
  const inheritedBySource = duplicateCandidates.rows.reduce<Record<string, number>>(
    (result, row) => {
      result[row.source_key] = (result[row.source_key] ?? 0) + 1;
      return result;
    },
    ...
    JSON.stringify(
      {
        ...
        missingInWarsawLast30DaysBySource: Object.fromEntries(
          recent.rows.map((row) => [
            row.source_key,
            {
              count: Number(row.missing),
              percent:
                recentMissingTotal > 0
                  ? Number(((Number(row.missing) / recentMissingTotal) * 100).toFixed(1))
                  : 0,
            },
          ]),
        ),
