...
import { pool } from "../../db";
import { resolveArchivedFile } from "../../services/archive/archived-file-reader";
...
try {
  ...
  const report = {
    mode: "apply",
    crawlArtifacts: {
      ...summarize(artifactCandidates, artifacts),
      updated: artifactResult.updated,
    },
    listingSnapshots: {
      ...summarize(snapshotCandidates, snapshots),
      updated: snapshotResult.updated,
    },
    logicalBytesRemoved: artifactResult.logicalBytesRemoved + snapshotResult.logicalBytesRemoved,
    logicalSizeRemoved: formatBytes(
      artifactResult.logicalBytesRemoved + snapshotResult.logicalBytesRemoved,
    ),
    logPath,
    nextStep:
      "VACUUM FULL odzyskuje miejsce dla systemu plików, ale wymaga wyłącznego locka i należy uruchomić go osobno w oknie serwisowym.",
  };
  await appendFile(
    logPath,
...
async function compactTable(
  table: "crawl_artifacts" | "listing_snapshots",
  ...
  for (let index = 0; index < candidates.length; index += 250) {
    ...
    logicalBytesRemoved += batch.reduce(
      (total, candidate) => total + candidate.reclaimable_bytes,
      0,
    );
  }
...
function summarize(
  all: CandidateRow[],
  result: { verified: CandidateRow[]; missing: CandidateRow[] },
) {
  const reclaimableBytes = result.verified.reduce(
    (total, candidate) => total + candidate.reclaimable_bytes,
    0,
  );
  return {
