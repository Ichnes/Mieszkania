...
type SnapshotCandidate = { externalId: string; timestampDir: string; rawPath: string; parsedPath: string };
...
async function main() {
  ...
  for (const candidate of candidates) {
    try {
      ...
      const previousParsed = JSON.parse(await readArchivedText(candidate.parsedPath)) as { canonicalUrl?: string; description?: string };
      const canonicalUrl = previousParsed.canonicalUrl ?? `https://www.domiporta.pl/nieruchomosci/${candidate.externalId.replace("domiporta-", "")}`;
      const parsed = parseListing(canonicalUrl, rawHtml, candidate.externalId, rawHtml.replace(/<[^>]+>/g, " "), previousParsed.description);
