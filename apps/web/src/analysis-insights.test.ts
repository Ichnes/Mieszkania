...
import { groupSurroundings, analyzeDescription } from "./analysis-insights";
...
test("rail segments collapse by type, keep nearest distance and do not hide waste", () => {
  const rails: ImmediateSurroundingsFinding[] = Array.from({ length: 20 }, (_, id) => ({ osmKey: `way:${id}`, category: "railway", name: "Tory", label: "Tory", distanceMeters: id + 1, severity: "information", detail: "railway=rail", osmUrl: `https://www.openstreetmap.org/way/${id}` }));
  const waste: ImmediateSurroundingsFinding = { ...rails[0], osmKey: "node:200", category: "waste", label: "Odpady", name: "Recykling", distanceMeters: 40 };
...
test("description questions retain original evidence and avoid simple negations", () => {
  ...
  assert.deepEqual(insights.map((item) => item.key), ["extras"]);
