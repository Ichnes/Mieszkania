...
export function getSunExposure(description?: string) {
  ...
  const exposureText = text
    ...
    .replace(/\b(?:obwodnic\w*|tras\w*|alej\w*|arteri\w*|drog\w*)\s+(?:poludni|polnoc|wschod|zachod)\w*/g, " ")
    .replace(/\b(?:poludni|polnoc|wschod|zachod)\w*\s+(?:obwodnic\w*|tras\w*|alej\w*|arteri\w*|drog\w*)/g, " ");
  const directions = new Set<ExposureDirection>();
  (exposureText.match(/\b(?:NE|NW|SE|SW)\b/g) ?? []).forEach((direction) => directions.add(direction as ExposureDirection));
  ...
  const compoundExposures = pairDirections
    .flatMap(([pattern, direction]) => Array.from(remaining.matchAll(pattern), (match) => ({
      direction,
      index: match.index ?? 0
    })))
  ...
  const cardinalDirections: Record<string, ExposureDirection> = { poludni: "S", pld: "S", polnoc: "N", wschod: "E", zachod: "W", zach: "W" };
  for (const match of remaining.matchAll(/\b(poludni\w*|pld|polnoc\w*|wschod\w*|zachod\w*|zach)\b/g)) {
    const index = match.index ?? 0;
    const context = remaining.slice(Math.max(0, index - 70), index + match[0].length + 70);
    const hasExposureContext = /\b(?:ekspozycj\w*|orientacj\w*|wystaw\w*|okn\w*|balkon\w*|taras\w*|salon\w*|sypialn\w*|przedpokoj\w*|pokoj\w*|widok\w*|wychodz\w*|skierowan\w*|usytuowan\w*|dwustron\w*|od\s+strony|od)\b/.test(context);
    if (!hasExposureContext) continue;
    const prefix = Object.keys(cardinalDirections).find((candidate) => match[1].startsWith(candidate));
...
export function getDescriptionHighlightParts(value: string): DescriptionHighlightPart[] {
  ...
  const importantPattern = new RegExp(`(${contextualPremium}|${contextualPositive}|${contextualNeutral}|${negative}|${positive}|${neutral})`, "giu");
  ...
  const hasExposureContext = /\b(?:ekspozycj\w*|orientacj\w*|okn\w*|dwustron\w*|od\s+(?:wschod|zachod|polnoc|poludni))\b/.test(normalizeListingText(value));
  const standaloneDirectionPattern = /^(?:wschód|zachód|południe|północ)$/iu;
  const directionalAdjectivePattern = /^(?:zachodni|wschodni|północn|południow)[a-ząćęłńóśźżą-]*$/iu;

  return value.split(importantPattern).filter(Boolean).map((text) => ({
    text,
    tone: negativePattern.test(text)
      ...
      : premiumPattern.test(text)
        ...
        : positivePattern.test(text)
          ? "positive"
          : (neutralPattern.test(text) || directionalAdjectivePattern.test(text) && hasExposureContext)
            && (!standaloneDirectionPattern.test(text) || hasExposureContext) ? "neutral" : undefined
  }));
}
