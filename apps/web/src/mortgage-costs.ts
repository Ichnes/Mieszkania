...
export const mortgageInsurancePresets: MortgageInsurancePreset[] = [
  {
    ...
    description: "Wpisz oprocentowanie, prowizję, wycenę i miesięczne stawki polis z formularza banku.",
    lifeMonthlyRate: 0,
...
export function calculateBankCosts(input: {
  ...
  if (input.principal <= 0) return {
    firstInsuranceMonthly: 0, lifeTotal: 0, propertyTotal: 0, commission: 0,
    valuation: 0, mortgageTax: 0, mortgageEntry: 0, oneOff: 0, insuranceTotal: 0
  };
  ...
  const lifeTotal = input.includeLife
    ? input.rows.reduce((sum, row) => sum + (row.balance + row.principal + row.extra) * input.preset.lifeMonthlyRate, 0)
    : 0;
  const propertyMonthly = input.includeProperty ? input.principal * input.preset.propertyMonthlyRate : 0;
...
export function calculatePurchaseCosts(input: {
  ...
  const purchaseTax = input.marketType === "secondary" && !input.firstHomeExemption
