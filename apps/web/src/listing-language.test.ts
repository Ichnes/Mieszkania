...
test("detects east-west exposure from the actual two-sided apartment description", () => {
  const result = getSunExposure("Mieszkanie dwustronne - od wschodu ulica Mickiewicza i widok na plac Wilsona; od zachodu widok na park.");
...
test("highlights whole bezczynszowe word without turning it into czynszowe", () => {
  const highlighted = getDescriptionHighlightParts("Mieszkanie bezczynszowe z tarasem.").filter((part) => part.tone);

  assert.deepEqual(highlighted.map((part) => [part.text, part.tone]), [
...
test("highlights useful apartment features in their common inflections", () => {
  const text = "Do lokalu przynależy piwnica, ogródek, 2 naziemne miejsca postojowe i drewniane podłogi.";
...
test("highlights the full administrative rent phrase", () => {
  const highlighted = getDescriptionHighlightParts("Czynsz administracyjny: 900 zł miesięcznie.").filter((part) => part.tone);
...
test("treats western exposure as direction but ignores time of day and sunsets", () => {
  const text = "Mieszkanie ma zachodnią ekspozycję, dzięki czemu popołudniami jest pięknie oświetlone, a z balkonu można podziwiać zachody słońca.";
  const result = getSunExposure(text);
  const highlighted = getDescriptionHighlightParts(text).filter((part) => part.tone === "neutral").map((part) => part.text);
...
test("understands colon-separated and window exposure descriptions", () => {
  assert.deepEqual(new Set(getSunExposure("Ekspozycja: południe (salon, kuchnia) i północ (dwie sypialnie).").directions), new Set(["S", "N"]));
  assert.deepEqual(new Set(getSunExposure("Ekspozycja okien-wschód, zachód.").directions), new Set(["E", "W"]));
});
...
test("does not turn no-renovation or obligatory parking into negative claims", () => {
  const noRenovation = getDescriptionHighlightParts("Mieszkanie nie wymaga remontu ani odświeżania.").filter((part) => part.tone);
  const obligatoryParking = getDescriptionHighlightParts("Zakup obligatoryjny - nie ma możliwości zakupu mieszkania bez miejsca postojowego.").filter((part) => part.tone);

  assert.deepEqual(noRenovation.map((part) => part.tone), ["positive"]);
  assert.deepEqual(obligatoryParking.map((part) => part.tone), ["neutral"]);
});

test("highlights compound floor, count and area expressions in full", () => {
  const highlighted = getDescriptionHighlightParts("Piętro 5/6, lokal na 3 i 4 piętrze, dwa balkony, dwa miejsca postojowe i 8-metrowy taras.")
    .filter((part) => part.tone)
    .map((part) => part.text.toLowerCase());

  assert.deepEqual(highlighted, ["piętro 5/6", "na 3 i 4 piętrze", "dwa balkony", "dwa miejsca postojowe", "8-metrowy", "taras"]);
});

test("highlights premium materials, systems and design phrases", () => {
  const text = "Garażu podziemnym, klimatyzowany salon, ogrzewanie podłogowe i wnętrze zaaranżowane przez architekta. Podłogi wykonano z gresu i egzotycznego drewna merbau.";
  const positive = getDescriptionHighlightParts(text).filter((part) => part.tone === "positive").map((part) => part.text.toLowerCase());
  const premium = getDescriptionHighlightParts(text).filter((part) => part.tone === "premium").map((part) => part.text.toLowerCase());

  assert.deepEqual(positive, ["garażu podziemnym"]);
  assert.deepEqual(premium, ["klimatyzowany salon", "ogrzewanie podłogowe", "zaaranżowane przez architekta", "egzotycznego drewna merbau"]);
});

test("treats smoked oak on floors as a premium wooden floor", () => {
  const highlighted = getDescriptionHighlightParts("Na podłogach znajduje się dąb wędzony oraz wysokiej jakości gres.")
    .filter((part) => part.tone);
...
test("highlights an oak floorboard as a premium material", () => {
  const highlighted = getDescriptionHighlightParts("W salonie położono dębową deskę.")
    .filter((part) => part.tone);

  assert.deepEqual(highlighted.map((part) => [part.text, part.tone]), [["dębową deskę", "premium"]]);
});

test("does not treat blackout blinds and full blackout as a dark-apartment drawback", () => {
  const highlighted = getDescriptionHighlightParts("Duże okno z żaluzją zaciemniającą zapewnia prywatność i możliwość pełnego zaciemnienia.");

  assert.equal(highlighted.some((part) => part.tone === "negative"), false);
});

test("does not treat darker floor panels as a dark-apartment drawback", () => {
  const highlighted = getDescriptionHighlightParts("W części dziennej jasne panele, w sypialniach ciemniejsze panele.");

  assert.equal(highlighted.some((part) => part.tone === "negative"), false);
});

test("still treats a dark apartment or room as a drawback", () => {
  const highlighted = getDescriptionHighlightParts("Mieszkanie jest ciemne, a pokój ciemniejszy niż na zdjęciach.");

  assert.equal(highlighted.filter((part) => part.tone === "negative").length, 2);
});

test("highlights a direct offer from the owner", () => {
  const highlighted = getDescriptionHighlightParts("Bezpośrednio od właściciela.").filter((part) => part.tone);

  assert.deepEqual(highlighted.map((part) => [part.text, part.tone]), [["Bezpośrednio od właściciela", "positive"]]);
});
