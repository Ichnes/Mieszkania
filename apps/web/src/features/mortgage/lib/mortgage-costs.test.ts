...
test("cash purchase has no loan valuation, policies or mortgage fees", () => {
  const result = calculateBankCosts({
    principal: 0,
    rows: [],
    preset: mortgageInsurancePresets.find((item) => item.key === "ing_basic")!,
    includeLife: true,
...
test("custom commission and valuation are included in upfront bank costs", () => {
  const preset = {
    ...mortgageInsurancePresets.find((item) => item.key === "custom")!,
    commissionRate: 0.02,
    valuationFee: 500,
  };
...
test("applies PCC only to a non-exempt secondary-market purchase", () => {
  const taxed = calculatePurchaseCosts({
    propertyPrice: 800_000,
    marketType: "secondary",
    firstHomeExemption: false,
  });
  const exempt = calculatePurchaseCosts({
    propertyPrice: 800_000,
    marketType: "secondary",
    firstHomeExemption: true,
  });
...
test("bank scenario includes declining-balance life insurance and fixed property insurance", () => {
  ...
  const result = calculateBankCosts({
    principal: 100_000,
    rows: [
      { balance: 99_000, principal: 1_000, extra: 0 },
      { balance: 98_000, principal: 1_000, extra: 0 },
    ],
    preset,
    includeLife: true,
    includeProperty: true,
