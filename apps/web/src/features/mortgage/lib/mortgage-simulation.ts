...
export function calculateMortgage(
  ...
  oneOffAmount = 0,
  oneOffMonth = 1,
  strategy: "shorten" | "lower_payment" = "shorten",
  rateChange?: { month: number; rate: number; transitionMonths: number },
) {
  ...
  const annuity = (balance: number, rate: number, count: number) =>
    rate === 0 ? balance / count : (balance * rate) / (1 - (1 + rate) ** -count);
  ...
  for (let month = 1; month <= months && balance > 0.01; month += 1) {
    const progress = rateChange
      ? Math.min(
          1,
          Math.max(0, month - Math.max(1, rateChange.month) + 1) /
            Math.max(1, rateChange.transitionMonths),
        )
      : 0;
    const effectiveRate = rateChange
      ? annualRate + (nonNegative(rateChange.rate) - annualRate) * progress
      : annualRate;
    const rate = effectiveRate / 1200;
    if (strategy === "lower_payment" || effectiveRate !== previousRate)
      scheduledPayment = annuity(balance, rate, months - month + 1);
    previousRate = effectiveRate;
    const interest = balance * rate;
    const capital =
      month === months ? balance : Math.min(balance, Math.max(0, scheduledPayment - interest));
    const extra = Math.min(
      Math.max(0, balance - capital),
      monthlyExtra + (month === oneOffMonth ? oneOffAmount : 0),
    );
