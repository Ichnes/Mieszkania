...
export function analyzeDescription(description: string) {
  ...
  const rules = [
    {
      ...
      match: /(?:dodatkowo platn|obligatoryjn|obowiazkow.{0,25}zakup|prowizj|czynsz.{0,25}\d)/,
      question: "Jaka jest pełna cena z parkingiem, komórką i prowizją oraz miesięczne opłaty?",
    },
    {
      key: "condition",
      label: "Stan i nakłady",
      match: /(?:do remontu|wymaga remontu|do odswiezenia|stan.{0,15}dewelopersk|do wymiany)/,
      question: "Jaki zakres prac i budżet są potrzebne przed zamieszkaniem?",
      exclude: /(?:nie wymaga|bez koniecznosci).{0,20}remont/,
    },
    {
      key: "ownership",
      label: "Dokumenty do sprawdzenia",
      match:
        /(?:ksieg.{0,15}wieczyst|spoldzielcz|uregulowan.{0,15}grunt|udzial.{0,15}nieruchom|sluzebn|dozywoci)/,
      question: "Poproś o dokument potwierdzający prawo do lokalu i informacje o obciążeniach.",
    },
    {
      key: "availability",
      label: "Termin i przekazanie",
      match:
        /(?:wynajet|najemc|lokator|dostepn.{0,15}od|wolne od|termin.{0,15}(?:wydania|przekazania))/,
      question: "Kiedy lokal zostanie przekazany i czy będzie wolny od najemców?",
      exclude: /(?:bez|brak)\s+(?:najemc|lokator)/,
    },
    {
      key: "access",
      label: "Dostępność budynku",
      match: /(?:bez windy|brak windy|nie ma windy|schod.{0,15}wejsc)/,
      question: "Sprawdź dojście, schody i możliwość wniesienia wózka podczas wizyty.",
    },
  ];
  return rules.flatMap((rule) => {
    ...
    return evidence
      ? [{ key: rule.key, label: rule.label, question: rule.question, evidence }]
      : [];
  });
}
