...
const data: MsiCollection = JSON.parse(
...
test("MSI snapshot has real closed polygons in Warsaw and all 18 districts", () => {
  ...
  for (const f of data.features)
    for (const polygon of geometryRings(f.geometry))
...
test("Wola Grzybowska belongs to Wesoła, not Wola", () => {
  assert.equal(
    data.features.find((f) => f.properties.name === "Wola Grzybowska")?.properties.district,
    "Wesoła",
  );
});
