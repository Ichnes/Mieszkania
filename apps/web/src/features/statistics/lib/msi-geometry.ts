...
export type MsiFeature = {
  id: number;
  properties: { name: string; district: string };
  geometry: ParcelGeometry;
};
export type MsiCollection = { features: MsiFeature[]; updatedAt: string };
export const projectMsiPoint = ([lon, lat]: number[]) => [
  ((lon - 20.76) / 0.56) * 1000,
  ((52.42 - lat) / 0.36) * 1000,
];
export const geometryRings = (geometry: ParcelGeometry) =>
  geometry.type === "Polygon" ? [geometry.coordinates] : geometry.coordinates;
export function geometryPath(geometry: ParcelGeometry) {
  return geometryRings(geometry)
    .flatMap((polygon) =>
      polygon.map((ring) => `M${ring.map((point) => projectMsiPoint(point).join(",")).join("L")}Z`),
    )
    .join("");
}
export function geometryBounds(geometries: ParcelGeometry[]) {
  const points = geometries.flatMap((g) =>
    geometryRings(g).flatMap((p) => p[0].map(projectMsiPoint)),
  );
  ...
  const padding = Math.max(width, height) * 0.07;
  return {
    x: x - padding,
    y: y - padding,
  ...
  for (const polygon of polygons) {
    ...
    for (let ix = 0; ix < 16; ix++)
      for (let iy = 0; iy < 16; iy++) {
        const point = [minX + (width * (ix + 0.5)) / 16, minY + (height * (iy + 0.5)) / 16];
